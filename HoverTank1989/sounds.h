#pragma once

// WLJ ToDo: sort out converting this to an enum class 
enum XACT_WAVEBANK_AUDIOBANK
{
    XACT_WAVEBANK_SOUNDS_KNIGHTRIDERMUSIC = 0,
    XACT_WAVEBANK_SOUNDS_COINSFX = 1,
    /*
    XACT_WAVEBANK_AUDIOBANK_MUSIC01 = 0,    // Racing Techno
    XACT_WAVEBANK_AUDIOBANK_MUSIC02 = 1,    // Snabich Tunes MXS
    XACT_WAVEBANK_AUDIOBANK_MUSIC03 = 2,    // In Summer
    XACT_WAVEBANK_AUDIOBANK_MUSIC04 = 3,    // Retroracing Beach
    XACT_WAVEBANK_AUDIOBANK_COINSFX = 4,    // Coin SFX prior to start scren
    XACT_WAVEBANK_AUDIOBANK_IMPACTSFX1 = 5, // Golf swing impact SFX
    */
};

#define XACT_WAVEBANK_AUDIOBANK_ENTRY_COUNT 2
//#define XACT_WAVEBANK_AUDIOBANK_ENTRY_COUNT 6

