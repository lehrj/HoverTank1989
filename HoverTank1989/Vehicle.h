...
struct HeliData
{
   ...
   // float hoverDriveOutputAudio = 0.0f;
    ...
    //const DirectX::SimpleMath::Vector3 turretTensorDimensions = DirectX::SimpleMath::Vector3(1.75f, 0.5f, 1.15f);
    const DirectX::SimpleMath::Vector3 turretTensorDimensions = DirectX::SimpleMath::Vector3(7.75f, 0.5f, 1.15f);
    const DirectX::SimpleMath::Vector3 turretTensorOffset = DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f);
    const float turretTensorMass = 100.0f;
...
public:
    ...
    void DrawVehicleProjectiles2Demo(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout);

    void FireWeapon();
    void FireWeapon(std::shared_ptr<DirectX::SoundEffectInstance> aFireFx);
    void FireWeapon(std::shared_ptr<Utility::SoundFx> aFireFx);
    void FireMissile(std::shared_ptr<Utility::SoundFx> aFireFx, std::shared_ptr<Utility::SoundFx> aRocketFx);
    void FireMissileDemo(std::shared_ptr<Utility::SoundFx> aFireFx, std::shared_ptr<Utility::SoundFx> aRocketFx);
