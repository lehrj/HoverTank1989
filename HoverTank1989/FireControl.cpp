...
Utility::ForceAccum FireControl::BoosterAccum(MissileData& aMissile)
{
    ...
    auto forcePosWorld = forcePos;
    forcePosWorld = DirectX::SimpleMath::Vector3::Transform(forcePosWorld, aMissile.projectileData.alignmentQuat);
    forcePosWorld += aMissile.projectileData.q.position;

    auto forceDirWorld = forceDir;
    forceDirWorld = DirectX::SimpleMath::Vector3::Transform(forceDirWorld, aMissile.projectileData.alignmentQuat);

    m_debugData->ToggleDebugOnOverRide();
    //m_debugData->PushDebugLine(forcePosWorld, forceDirWorld, 5.0f, 0.0f, DirectX::Colors::Red);
    m_debugData->ToggleDebugOff();
...
Utility::ForceAccum FireControl::FinForceAccum(const FinDataStatic& aFinLib, const FinDataDynamic& aFinDyn, const MissileData& aMissile)
{
    ...
    auto posWorld = forcePos;
    posWorld = DirectX::SimpleMath::Vector3::Transform(posWorld, aMissile.projectileData.alignmentQuat);
    posWorld += aMissile.projectileData.q.position;

    auto dirWorld = forceDir;
    dirWorld = DirectX::SimpleMath::Vector3::Transform(dirWorld, aMissile.projectileData.alignmentQuat);
    ...
    m_debugData->DebugPushUILineDecimalNumber("forceDirXXX = ", forceDir.Length(), "");
    m_debugData->PushDebugLine(posWorld, dirWorld, 4.0f, 0.0f, DirectX::Colors::White);
    //m_debugData->PushDebugLine(aMissile.projectileData.q.position, dirWorld, 3.0f, 0.1f, DirectX::Colors::Blue);

    m_debugData->ToggleDebugOff();

    //DebugPushDrawData(forcePos, forceDir);
    //DebugPushDrawData(aFinLib.posLocal, aFinLib.finNormal);
    //DebugPushDrawData(aFinLib.posLocal, aFinDyn.liftForce);
    //DebugPushDrawData(aFinLib.posLocal, aFinDyn.resultantForce);
    return accum;
}
...
void FireControl::PrintMissileData(MissileData& aMissile, const float aTimeDelta)
{
    ...
    //m_debugData->DebugPushUILineDecimalNumber("aMissile.guidance.linearForceSum        = ", aMissile.guidance.linearForceSum.Length(), "");
    //m_debugData->DebugPushUILineDecimalNumber("aMissile.guidance.linearDragSum         = ", aMissile.guidance.linearDragSum.Length(), "");
    ...
    //m_debugData->DebugPushUILineDecimalNumber("aMissile.projectileData.angularDragSum  = ", aMissile.projectileData.angularDragSum.Length(), "");

    //m_debugData->PushDebugLine(aMissile.projectileData.q.position, aMissile.guidance.linearForceSum, 9.0f, 0.0f, DirectX::Colors::Yellow);
    //m_debugData->PushDebugLine(aMissile.projectileData.q.position, aMissile.guidance.linearDragSum, 8.0f, 0.0f, DirectX::Colors::Red);

    auto angularForceSumWorld = aMissile.projectileData.angularForceSum;
    angularForceSumWorld = DirectX::SimpleMath::Vector3::Transform(angularForceSumWorld, aMissile.projectileData.alignmentQuat);

    auto angularDragSumWorld = aMissile.projectileData.angularDragSum;
    angularDragSumWorld = DirectX::SimpleMath::Vector3::Transform(angularDragSumWorld, aMissile.projectileData.alignmentQuat);


    //m_debugData->PushDebugLine(aMissile.projectileData.q.position, angularForceSumWorld, 7.0f, 0.0f, DirectX::Colors::Blue);
    //m_debugData->PushDebugLine(aMissile.projectileData.q.position, angularDragSumWorld, 6.0f, 0.0f, DirectX::Colors::Orange);
...
void FireControl::RightHandSideMissile(struct MissileData* aProjectile, ProjectileMotion* aQ, ProjectileMotion* aDeltaQ, double aTimeDelta, float aQScale, ProjectileMotion* aDQ)
{
    ...
    DirectX::SimpleMath::Vector3 drag = airResistance;
    DirectX::SimpleMath::Vector3 dragNew = CalcDragLinearCurrent(aProjectile, newQ.velocity);
...
void FireControl::UpdateFinData(MissileData& aMissile)
{
    ...
    //UpdateFinForces(m_finLib.mainPitch, aMissile.guidance.finPak.mainPitch, localVel, alignQuat, aMissile);
    //UpdateFinForces(m_finLib.mainYaw, aMissile.guidance.finPak.mainYaw, localVel, alignQuat, aMissile);
}
...
void FireControl::UpdateFinForces(const FinDataStatic& aStaticDat, FinDataDynamic& aFinDyn, const DirectX::SimpleMath::Vector3 aVelLocal, const DirectX::SimpleMath::Quaternion aAlignQuat, const MissileData& aMissile)
{
    ...
    //angleOfAttack = angleOfAttackWrapped;
    m_debugData->ToggleDebugOnOverRide();
    //m_debugData->DebugPushUILineDecimalNumber("angleOfAttack           = ", Utility::ToDegrees(angleOfAttack), "");
    //m_debugData->DebugPushUILineDecimalNumber("angleOfAttackWrapped    = ", Utility::ToDegrees(angleOfAttackWrapped), "");
    m_debugData->DebugPushUILineDecimalNumber("angleOfAttackTest   = ", Utility::ToDegrees(angleOfAttackTest), "");
    m_debugData->DebugPushUILineWholeNumber("angleOfAttackTest   = ", static_cast<int>(Utility::ToDegrees(angleOfAttackTest)), "");
    m_debugData->DebugPushUILineDecimalNumber("finAngle = ", finAngle, "");
    ...
    */
    ...
    //DebugPushDrawData(aStaticDat.posLocal, aStaticDat.finNormal);

}

void FireControl::UpdateFinForcesOld(const FinDataStatic& aStaticDat, FinDataDynamic& aFinDyn, const DirectX::SimpleMath::Vector3 aVelLocal, const DirectX::SimpleMath::Quaternion aAlignQuat, const MissileData& aMissile)
{
    auto airSpeedLocal = aVelLocal * -1.0f;
    auto airSpeedLocalNorm = airSpeedLocal;
    airSpeedLocalNorm.Normalize();
    ...
    /*
    m_debugData->DebugPushUILineDecimalNumber("finAxis.x = ", finAxis.x, "");
    m_debugData->DebugPushUILineDecimalNumber("finAxis.y = ", finAxis.y, "");
    m_debugData->DebugPushUILineDecimalNumber("finAxis.z = ", finAxis.z, "");
    */
    m_debugData->DebugPushUILineDecimalNumber("finVec.x = ", finVec.x, "");
    m_debugData->DebugPushUILineDecimalNumber("finVec.y = ", finVec.y, "");
    m_debugData->DebugPushUILineDecimalNumber("finVec.z = ", finVec.z, "");

    m_debugData->DebugPushUILineDecimalNumber("airSpeedLocalNorm.x = ", airSpeedLocalNorm.x, "");
    m_debugData->DebugPushUILineDecimalNumber("airSpeedLocalNorm.y = ", airSpeedLocalNorm.y, "");
    m_debugData->DebugPushUILineDecimalNumber("airSpeedLocalNorm.z = ", airSpeedLocalNorm.z, "");
...
void FireControl::UpdateMissileVec(double aTimeDelta)
{
    for (unsigned int i = 0; i < m_missileVec.size(); ++i)
    {
        ...
        //m_debugData->ToggleDebugOnOverRide();
        m_debugData->DebugPushUILineDecimalNumber("tailPitch =  ", m_missileVec[i].guidance.finPak.tailPitch.controlInput, "");
        m_debugData->DebugPushUILineDecimalNumber("tailYaw   =  ", m_missileVec[i].guidance.finPak.tailYaw.controlInput, "");
...
void FireControl::UpdateControlData(MissileData& aMissile, const float aTimeDelta)
{
    ...
    m_debugData->PushDebugLine(aMissile.projectileData.q.position, navVecWorld, 500.0f, 0.0f, DirectX::Colors::Violet);
