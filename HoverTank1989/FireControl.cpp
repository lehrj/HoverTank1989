...
void FireControl::ActivateMuzzleFlash(AmmoType aAmmoType)
{
    ...
    else if (aAmmoType == AmmoType::AMMOTYPE_GUIDEDMISSILE)
    {
        ...
        if (m_tubeFireSelected == MissileTubeSelected::MISSILETUBESELECTED_LEFT)
        {
        ...
        {
            mf.flashType = MuzzleFlashType::MUZZLEFLASHTYPE_RIGHT;
...
void FireControl::DeleteMissileFromVec(const unsigned int aIndex)
{
    ...
    else
    {
        ...
        //m_missileVec[aIndex].audioFx->fx->Stop();
...
void FireControl::DeleteMissileFromVecWithSound(const unsigned int aIndex)
{
    if (aIndex > m_missileVec.size())
    {
        // add error handling here
...
void FireControl::DrawContrails(MissileData& aMissile)
{
    ...
    unsigned int loopMax = aMissile.guidance.contrailLength;
    if (loopMax > 0)
    {
        --loopMax;
    }
    ...
    for (unsigned int i = loopMax; i > 0; --i)
    {
        ...
        if (aMissile.guidance.isContrailOn == true)
        {
...
void FireControl::DrawFireControlObjects2(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    ...
    DrawMuzzleFlashVec(aView, aProj, aEffect, aInputLayout);
    if (m_muzzleFlashVec.size() > 0)
    {
        //DrawMuzzleFlashVec(aView, aProj, aEffect, aInputLayout);
    }
...
{
    DrawMissilesWithLightingDemo(aView, aProj, aEffect, aInputLayout);
}
...
void FireControl::DrawLaser(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    ...
    if (m_isTargetingLaserOn == true)
    {
        ...
        */
        ...
        aEffect->SetAmbientLightColor(laserColor);
    ...
    if (m_missileConsts.isMissleTargetingLaserTrue == true && m_isLaserMissileTrue == true)
    {
        ...
        for (unsigned int i = 0; i < m_missileVec.size(); ++i)
        {
            if (m_missileVec[i].guidance.isTargetingLaserOn == true && m_missileVec[i].guidance.isTargetLocked == true)
            {
                ...
                if (m_missileVec[i].guidance.isLaserFlickerTrue == true)
                {
                    m_missileVec[i].guidance.isLaserFlickerTrue = false;
                }
                else
                {
...
void FireControl::DrawLaserOld(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    if (m_isTargetingLaserOn == true)
    {
        float scale = m_playerLaser.distance;
...
void FireControl::DrawMissilesWithLighting(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    ...
    groundPlane.Normalize();
    const DirectX::SimpleMath::Matrix shadowMat = DirectX::SimpleMath::Matrix::CreateShadow(lightDir, groundPlane);
    ...
    for (unsigned int i = 0; i < m_missileVec.size(); ++i)
    {
        ...
        const float testRand = abs(cos(m_missileVec[i].projectileData.time * m_ammoMissile.modelData.afterBurnFlickerRateScale)) * 0.05f;
        scale -= testRand;
        ...
        m_missileVec[i].guidance.afterBurnPlumeSphereMat = sphereMat;
        ...
        // end afterburner and jet lighting
        /////////////////////////////////////////////////////////////////////
        ...
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationY(m_missileVec[i].guidance.finPak.tailYaw.finAngle);
        updateMat *= m_ammoMissile.modelData.tailFinTransMat;
        const DirectX::SimpleMath::Matrix finMat1 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        ...
        updateMat *= m_ammoMissile.modelData.tailFinTransMat;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(180.0f));
        const DirectX::SimpleMath::Matrix finMat2 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= m_ammoMissile.modelData.tailFinTransMat;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(90.0f));
        const DirectX::SimpleMath::Matrix finMat3 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= m_ammoMissile.modelData.tailFinTransMat;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(270.0f));
        const DirectX::SimpleMath::Matrix finMat4 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationY(-wingFinDeployAngle + Utility::ToRadians(180.0f));
        updateMat *= m_ammoMissile.modelData.wingTranslation;
        const DirectX::SimpleMath::Matrix wingMat1 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= m_ammoMissile.modelData.wingTranslation;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(90.0f));
        const DirectX::SimpleMath::Matrix wingMat2 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= m_ammoMissile.modelData.wingTranslation;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(180.0f));
        const DirectX::SimpleMath::Matrix wingMat3 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        updateMat *= m_ammoMissile.modelData.wingTranslation;
        updateMat *= DirectX::SimpleMath::Matrix::CreateRotationX(Utility::ToRadians(-90.0f));
        const DirectX::SimpleMath::Matrix wingMat4 = updateMat;
        updateMat *= alignRotMat;
        updateMat *= posTransMat;
        aEffect->SetWorld(updateMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.finColor1);
        ...
        // shadows start
        DirectX::SimpleMath::Vector3 modelPos = m_missileVec[i].projectileData.q.position;
        const float altitude = m_missileVec[i].projectileData.q.position.y;
        ...
        const DirectX::SimpleMath::Matrix shadowDrawMatBase = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_missileVec[i].projectileData.alignmentQuat);
        DirectX::SimpleMath::Matrix shadowDrawMat = shadowDrawMatBase;
        DirectX::SimpleMath::Matrix alignMat = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_missileVec[i].projectileData.alignmentQuat);

        updateMat = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_missileVec[i].projectileData.alignmentQuat);
        updateMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position);

        // main body
        shadowDrawMat = m_ammoMissile.modelData.localBodyMatrix;
        shadowDrawMat *= DirectX::SimpleMath::Matrix::Identity * inverseShadowScale;
        shadowDrawMat *= shadowScaleMat;
        shadowDrawMat *= alignRotMat;
        shadowDrawMat *= posTransMat;
        ...
        // wing 1
        shadowDrawMat = wingMat1;
        shadowDrawMat *= alignMat;
        shadowDrawMat *= inverseShadowScale;
        shadowDrawMat *= shadowScaleMat;
        shadowDrawMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position);;
        shadowDrawMat *= shadowMat;
        aEffect->SetWorld(shadowDrawMat);
        m_ammoMissile.modelData.wingFinShape->Draw(aEffect.get(), aInputLayout.Get());

        // wing 2
        shadowDrawMat = wingMat2;
        shadowDrawMat *= alignMat;
        shadowDrawMat *= inverseShadowScale;
        shadowDrawMat *= shadowScaleMat;
        shadowDrawMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position);;
...
void FireControl::DrawMissilesWithLightingDemo(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    ...
    aEffect->SetDiffuseColor(DirectX::SimpleMath::Vector3(1.0f, 1.0f, 1.0f));
    aEffect->SetEmissiveColor(DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f));
    aEffect->SetAmbientLightColor(DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f));
    aEffect->SetSpecularColor(DirectX::SimpleMath::Vector3(1.0f, 1.0f, 1.0f));
    aEffect->SetAlpha(0.0f);
...
void FireControl::DrawMissilesWithLightingOld(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    for (unsigned int i = 0; i < m_missileVec.size(); ++i)
    {
        ...
        const float baseBurnRotation = m_missileVec[i].guidance.afterBurnFlickerRotation;
        ...
        */
        ...
        testRand2 = testRand * 2.0f;

        auto diffuseMod = diffuse0;
        diffuseMod.x *= m_missileVec[i].guidance.throttlePercentage - testRand2;
        diffuseMod.y *= m_missileVec[i].guidance.throttlePercentage - testRand2;
        diffuseMod.z *= m_missileVec[i].guidance.throttlePercentage - testRand2;
        auto specMod = specular0;
        specMod.x *= m_missileVec[i].guidance.throttlePercentage - testRand2;
        specMod.y *= m_missileVec[i].guidance.throttlePercentage - testRand2;
        specMod.z *= m_missileVec[i].guidance.throttlePercentage - testRand2;

        //auto colorInput = 1.0f - testRand2;
        auto colorInput = m_missileVec[i].guidance.throttlePercentage - testRand;
        DirectX::SimpleMath::Vector4 plumeLightingColor = DirectX::SimpleMath::Vector4(colorInput, colorInput, colorInput, colorInput);

        //aEffect->SetLightDiffuseColor(0, plumeLightingColor);
        //aEffect->SetLightSpecularColor(0, plumeLightingColor);
        aEffect->SetLightDiffuseColor(1, diffuseMod);
        aEffect->SetLightSpecularColor(1, specMod);
        ...
        */
        ...
        //const float altitude = m_missileVec[i].projectileData.q.position.y - m_environment->GetTerrainHeightAtPos(m_missileVec[i].projectileData.q.position);
        float altitude = m_missileVec[i].projectileData.q.position.y;
        float testAlt = altitude;
        float terrainHeightAtPos = 0.0f;
        float terrainHeightAtPos2 = 0.0f;
        DirectX::SimpleMath::Plane groundPlane2;
        DirectX::SimpleMath::Vector3 terrainNorm = DirectX::SimpleMath::Vector3::UnitY;
        //bool isShadowBoolTrue = m_environment->GetVehicleUpdateData(m_missileVec[i].projectileData.q.position, terrainNorm, testAlt, groundPlane);
        //bool isShadowBoolTrue = m_environment->GetVehicleUpdateData(m_missileVec[i].projectileData.q.position, terrainNorm, terrainHeightAtPos, groundPlane);
        //bool isVehicleInPlayUpdate = m_vehicleStruct00.environment->GetVehicleUpdateData(m_vehicleStruct00.vehicleData.q.position, m_vehicleStruct00.vehicleData.terrainNormal, m_vehicleStruct00.vehicleData.terrainHightAtPos, m_vehicleStruct00.vehicleData.groundPlane);
        bool isShadowBoolTrue = m_environment->GetVehicleUpdateData(m_missileVec[i].projectileData.q.position, terrainNorm, terrainHeightAtPos2, groundPlane2);

        planeTrans = DirectX::SimpleMath::Matrix::Identity;
        planeTrans *= DirectX::SimpleMath::Matrix::CreateTranslation(zFightOffSet);
        planeTrans = planeTrans.Transpose();
        groundPlane2 = DirectX::SimpleMath::Plane::Transform(groundPlane2, planeTrans);
        groundPlane2.Normalize();

        DirectX::SimpleMath::Matrix shadowMat = DirectX::SimpleMath::Matrix::CreateShadow(lightDir, groundPlane2);
        //altitude = m_missileVec[i].projectileData.q.position.y - terrainHeightAtPos + 1.2;
        altitude = m_missileVec[i].projectileData.q.position.y;
        if (altitude > maxShadowRange)
        {
            shadowScale = 0.0f;
            inverseShadowScale = 1.0f;
        ...
        //DirectX::SimpleMath::Matrix projMat = DirectX::SimpleMath::Matrix::Identity;

        //shadowScale = 25.0f;
        DirectX::SimpleMath::Matrix shadowScaleMat = DirectX::SimpleMath::Matrix::CreateScale(DirectX::SimpleMath::Vector3(shadowScale, shadowScale, shadowScale));
        DirectX::SimpleMath::Matrix shadowDrawMat = m_ammoMissile.modelData.localBodyMatrix;

        DirectX::SimpleMath::Vector3 shadowPos = m_missileVec[i].projectileData.q.position;
        shadowPos.y = terrainHeightAtPos2;
        //shadowPos.y += 1.0f;
        DirectX::SimpleMath::Matrix alignMat = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_missileVec[i].projectileData.alignmentQuat);
        projMat *= alignMat;
        //projMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position);
        shadowDrawMat = projMat;
        ...
        shadowDrawMat *= shadowScaleMat;
        //shadowDrawMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position);
        shadowDrawMat *= DirectX::SimpleMath::Matrix::CreateTranslation(shadowPos);
        //shadowDrawMat *= alignMat;
        shadowDrawMat *= shadowMat;
        ...
        testMat = testMat;
        testMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_missileVec[i].projectileData.q.position) * inverseShadowScale;
        testMat *= shadowScaleMat;
        testMat *= updateMat;
        testMat *= shadowMat;

        DirectX::SimpleMath::Vector3 scaleTest;
        DirectX::SimpleMath::Vector3 transTest;
        ...
        m_debugData->DebugPushUILineDecimalNumber("altitude ", altitude, "");
        m_debugData->DebugPushUILineDecimalNumber("testAlt ", testAlt, "");
        m_debugData->DebugPushUILineDecimalNumber("terrainHeightAtPos  ", terrainHeightAtPos, "");
        m_debugData->DebugPushUILineDecimalNumber("terrainHeightAtPos2 ", terrainHeightAtPos2, "");
        m_debugData->DebugPushUILineDecimalNumber("transTest.x ", transTest.x, "");
        m_debugData->DebugPushUILineDecimalNumber("transTest.y ", transTest.y, "");
        m_debugData->DebugPushUILineDecimalNumber("transTest.z ", transTest.z, "");
        m_debugData->ToggleDebugOff();

        aEffect->SetWorld(shadowDrawMat);
        //aEffect->SetWorld(testMat);
        aEffect->SetColorAndAlpha(m_ammoMissile.modelData.voidBlackColor);
        m_ammoMissile.modelData.mainBodyShape->Draw(aEffect.get(), aInputLayout.Get());
...
void FireControl::DrawMuzzleFlash(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj)
{
    ...
    m_muzzleFlashModel.muzzleFlashConeShape2->Draw(m_muzzleFlash.worldTestMatrix, aView, aProj, m_muzzleFlash.currentColor);
    m_muzzleFlashModel.muzzleFlashConeShape->Draw(m_muzzleFlash.worldMuzzleFlashConeMatrix, aView, aProj, m_muzzleFlash.currentColor);
}
...
void FireControl::DrawMuzzleFlash2(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    ...
    auto decompMat = m_muzzleFlash.worldTestMatrix;
    DirectX::SimpleMath::Vector3 transVec;
    DirectX::SimpleMath::Vector3 scaleVec;
    DirectX::SimpleMath::Quaternion quat;
    decompMat.Decompose(scaleVec, quat, transVec);

    auto muzzlePos = m_playerVehicle->GetMuzzlePos();
    ...
    m_muzzleFlashModel.muzzleFlashConeShape2->Draw(aEffect.get(), aInputLayout.Get());
    ...
    //m_muzzleFlashModel.muzzleFlashConeShape->Draw(aEffect.get(), aInputLayout.Get());

    aEffect->SetBiasedVertexNormals(false);
}

void FireControl::DrawMuzzleFlashVec(const DirectX::SimpleMath::Matrix aView, const DirectX::SimpleMath::Matrix aProj, std::shared_ptr<DirectX::NormalMapEffect> aEffect, Microsoft::WRL::ComPtr<ID3D11InputLayout> aInputLayout)
{
    for (unsigned int i = 0; i < m_muzzleFlashVec.size(); ++i)
    {
        aEffect->EnableDefaultLighting();

        DirectX::SimpleMath::Vector3 weaponDir = -m_playerVehicle->GetWeaponDirection();  // needs to be fixed
        DirectX::SimpleMath::Vector3 lightDir0 = weaponDir;
        DirectX::SimpleMath::Vector3 lightDir1 = weaponDir;
        DirectX::SimpleMath::Vector3 lightDir2 = weaponDir;

        const float flashDurationRatio = m_muzzleFlashVec[i].flashTimer / m_muzzleFlashVec[i].flashDuration;
        float coneSideAngle = atan((m_muzzleFlashVec[i].baseConeHeight / (m_muzzleFlashVec[i].baseConeDiameter * 0.5f)));
        const float coneSideAngleDegrees = Utility::ToDegrees(coneSideAngle);
        float lightAngle = flashDurationRatio * (Utility::GetPi() * -0.5f) + coneSideAngle;
        const float lightAngleDegrees = Utility::ToDegrees(lightAngle);
        ...
        if (m_muzzleFlashVec[i].isFlickerTrue == true)
        {
            Utility::GetDispersedLightDirectionsRotation(weaponDir, lightAngle, lightRotation, lightDir0, lightDir1, lightDir2);
        }
        else
        ...
        auto decompMat = m_muzzleFlashVec[i].worldTestMatrix;
        DirectX::SimpleMath::Vector3 transVec;
        DirectX::SimpleMath::Vector3 scaleVec;
        DirectX::SimpleMath::Quaternion quat;
        decompMat.Decompose(scaleVec, quat, transVec);
        ...
        aEffect->SetWorld(m_muzzleFlashVec[i].worldTestMatrix);
        aEffect->SetColorAndAlpha(m_muzzleFlashVec[i].currentColor);
        //m_muzzleFlash.muzzleFlashConeShape2->Draw(aEffect.get(), aInputLayout.Get());
        m_muzzleFlashModel.muzzleFlashConeShape2->Draw(aEffect.get(), aInputLayout.Get());

        //aEffect->SetWorld(m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix);
        //m_muzzleFlash.muzzleFlashConeShape->Draw(aEffect.get(), aInputLayout.Get());
        //m_muzzleFlashModel.muzzleFlashConeShape->Draw(aEffect.get(), aInputLayout.Get());
...
Utility::ImpulseForce FireControl::GetRecoilImpulseForce(DirectX::SimpleMath::Vector3 aDirectionNorm)
{
    ...
    {      
        recoilImpulseForce.impulseType = Utility::ImpulseType::IMPULSETYPE_LAGCURVE;
        recoilImpulseForce.currentTime = 0.0f;
...
void FireControl::GuidanceBasicGravityOld(MissileData& aMissile, const float aTimeDelta)
{
    const DirectX::SimpleMath::Vector3 selfPosW = aMissile.projectileData.q.position;
    const DirectX::SimpleMath::Vector3 selfVelW = aMissile.projectileData.q.velocity;
    const DirectX::SimpleMath::Vector3 targPosW = aMissile.guidance.targetPosition;
...
void FireControl::GuidanceClimbOut(MissileData& aMissile, const float aTimeDelta)
{
    ...
    float climbAng = Utility::GetAngleBetweenVectors(testVelNorm, upLocal);

    testVelNorm = DirectX::SimpleMath::Vector3::Transform(testVelNorm, DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(right, climbAng - Utility::ToRadians(45.0f)));
    ...
    /*
    m_debugData->DebugPushUILineDecimalNumber("climbAngleCurrent = ", Utility::ToDegrees(climbAngleCurrent), "");
    m_debugData->DebugPushUILineDecimalNumber("climbAngSet       = ", Utility::ToDegrees(climbAngSet), "");
    m_debugData->DebugPushUILineDecimalNumber("climbAng       = ", Utility::ToDegrees(climbAng), "");
    m_debugData->DebugPushUILineDecimalNumber("climbAngD       = ", Utility::ToDegrees(climbAng - Utility::ToRadians(45.0f)), "");
    */
    ...
    auto targVecNorm = testVelNorm;
    auto toTargQuat = DirectX::SimpleMath::Quaternion::FromToRotation(DirectX::SimpleMath::Vector3::UnitX, targVecNorm);
    toTargQuat.Normalize();
    toTargQuat.Inverse(toTargQuat);

    //DebugPushDrawData(DirectX::SimpleMath::Vector3::Zero, targVecNorm, DirectX::Colors::Orange, false, true);
    //DebugPushDrawData(DirectX::SimpleMath::Vector3::Zero, DirectX::SimpleMath::Vector3::UnitY, DirectX::Colors::White, false, false);
    //DebugPushDrawData(DirectX::SimpleMath::Vector3::Zero, upLocal, DirectX::Colors::Orange, false, true);
    //DebugPushDrawData(DirectX::SimpleMath::Vector3::Zero, targVecNorm, DirectX::Colors::Orange, false, true);
...
void FireControl::InitializeMissileInertiaTensor()
{
    ...
    */
    ...
    if (m_missileConsts.useAdvancedMoiTensorTrue == true)
    {
        ...
        const float altRadius = m_missileDimensions.z * 0.5f;
        const float altHeight = m_missileDimensions.x * 1.0f;
        const DirectX::SimpleMath::Vector3 altDimensions = DirectX::SimpleMath::Vector3(1.0f, 1.0f, 1.0f);
        const DirectX::SimpleMath::Vector3 altOffset = DirectX::SimpleMath::Vector3(0.0f, 0.0f, -0.5f);
        ...
        advancedTensor._33 = ((1.0f / 12.0f) * (mass) * (altHeight * altHeight)) + ((1.0f / 4.0f) * (mass) * (altRadius * altRadius));
        advancedTensor._22 = ((1.0f / 12.0f) * (mass) * (altHeight * altHeight)) + ((1.0f / 4.0f) * (mass) * (altRadius * altRadius));
        advancedTensor._11 = (1.0f / 2.0f) * (mass) * (altRadius * altRadius);
...
void FireControl::InitializeMuzzleFlashModel(Microsoft::WRL::ComPtr<ID3D11DeviceContext1> aContext, MuzzleFlash& aMuzzleFlash)
{
    ...
    m_muzzleFlashModel.muzzleFlashConeShape2 = DirectX::GeometricPrimitive::CreateCone(aContext.Get(), m_muzzleFlashModel.baseConeDiameter, m_muzzleFlashModel.baseConeHeight);
    m_muzzleFlashModel.muzzleFlashConeShape = DirectX::GeometricPrimitive::CreateSphere(aContext.Get(), m_muzzleFlashModel.baseConeDiameter, 16Ui64);

    m_muzzleFlashModel.localMuzzleConeMatrix = DirectX::SimpleMath::Matrix::Identity;
    m_muzzleFlashModel.worldMuzzleFlashConeMatrix = aMuzzleFlash.localMuzzleConeMatrix;

    m_muzzleFlashModel.worldTestMatrix = DirectX::SimpleMath::Matrix::Identity;
    m_muzzleFlashModel.localTestMatrix = DirectX::SimpleMath::Matrix::Identity;
}
...
void FireControl::InitializeProjectileModelMissile(Microsoft::WRL::ComPtr<ID3D11DeviceContext1> aContext, MissileStruct& aAmmo)
{
    ...
    const float seekerLensDiameter = m_playerLaser.missileLaserLensDiameter;
    ...
    aAmmo.modelData.rocketPlumeBaseShape = DirectX::GeometricPrimitive::CreateSphere(aContext.Get(), ammoDiameter, 64);
...
void FireControl::InitializeLaserModel(Microsoft::WRL::ComPtr<ID3D11DeviceContext1> aContext, LaserModel& aLazerModel)
{
    ...
    aLazerModel.laserShapeMissile = DirectX::GeometricPrimitive::CreateCylinder(aContext.Get(), length, 0.0304800030f);
...
void FireControl::LaserMissileSizeModOn()
{
    m_isLaserSizeModTrue = true;
}

void FireControl::LaserMissileSizeModOff()
{
    m_isLaserSizeModTrue = false;
}
...
void FireControl::PrintFlightStateData(MissileData& aMissile)
{
    ...
    {
        m_debugData->DebugPushUILineWholeNumber("FlightState::FLIGHTSTATE_OVERSHOOT ", 1, "");
    }
    else
    {
        m_debugData->DebugPushUILineWholeNumber("FlightState:: Unknown? ", 1, "");
    }
...
void FireControl::UpdateFireControl(double aTimeDelta)
{
    ...
    UpdateMuzzleFlashVec(aTimeDelta);
...
void FireControl::UpdateMissileModelData(MissileData& aMissile, const float aTimeDelta)
{
    ...
    auto targetPosLocalized = aMissile.guidance.targetPositionWorld - aMissile.projectileData.q.position;
    targetPosLocalized = DirectX::SimpleMath::Vector3::Transform(targetPosLocalized, aMissile.projectileData.inverseAlignmentQuat);
    ...
    toUseWorldMuzzlePos = DirectX::SimpleMath::Vector3::Zero;
    toUseWorldMuzzlePos.x = 2.0f;
    toUseWorldMuzzlePos.x = m_missileConsts.dimensions.x * 0.5f;
    toUseWorldMuzzlePos = DirectX::SimpleMath::Vector3::Transform(toUseWorldMuzzlePos, aMissile.projectileData.alignmentQuat);

    auto localSeekerPos = DirectX::SimpleMath::Vector3(m_missileConsts.dimensions.x * 0.5f, 0.0f, 0.0f);
    auto forwardToTargNormTest = targetPosLocalized;

    DirectX::SimpleMath::Vector3 forwardToTargNorm = m_npcController->GetNpcPos(aMissile.guidance.targetID) - (aMissile.projectileData.q.position + toUseWorldMuzzlePos);

    forwardToTargNorm.Normalize();
    forwardToTargNormTest.Normalize();

    if (forwardToTargNormTest.x < 0.0f)
    {
        forwardToTargNormTest.x = 0.0f;
        forwardToTargNormTest.Normalize();
    }
...
void FireControl::UpdateMissileVec(double aTimeDelta)
{
    for (unsigned int i = 0; i < m_missileVec.size(); ++i)
    {
        ...
        UpdateMissileModelData(m_missileVec[i], static_cast<float>(aTimeDelta));
...
void FireControl::UpdateMuzzleFlash(MuzzleFlash& aMuzzleFlash, const double aTimeDelta)
{
    for (unsigned int i = 0; i < m_muzzleFlashVec.size(); ++i)
    {
        m_muzzleFlashVec[i].flashTimer += static_cast<float>(aTimeDelta);

        m_muzzleFlashVec[i].flashTimer += static_cast<float>(aTimeDelta);
        if (m_muzzleFlashVec[i].flashTimer >= m_muzzleFlashVec[i].flashDuration)
        {
            m_muzzleFlashVec[i].isFlashActive = false;
            m_muzzleFlashVec[i].flashTimer = 0.0f;
            m_muzzleFlashVec[i].sizeMod = 0.0f;
            m_muzzleFlashVec[i].isDeleteTrue = true;
        }
        const float durationPercentage = m_muzzleFlashVec[i].flashTimer / m_muzzleFlashVec[i].flashDuration;
        const float inverseDurationPercentage = 1.0f - durationPercentage;
        if (durationPercentage <= 0.5f)
        {
            m_muzzleFlashVec[i].sizeMod += m_muzzleFlashVec[i].growthRate * static_cast<float>(aTimeDelta);
        }
        else
        {
            m_muzzleFlashVec[i].sizeMod -= m_muzzleFlashVec[i].growthRate * static_cast<float>(aTimeDelta);
        ...
        DirectX::SimpleMath::Vector4 colorTest = m_muzzleFlashVec[i].startColor - m_muzzleFlashVec[i].endColor;
        m_muzzleFlashVec[i].currentColor = m_muzzleFlashVec[i].startColor - (colorTest * durationPercentage);

        DirectX::SimpleMath::Matrix updateMat = m_playerVehicle->GetAlignment();
        updateMat *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetPos());
        const float maxModSize = m_muzzleFlashVec[i].flashDuration * m_muzzleFlashVec[i].growthRate;

        float scale = m_muzzleFlashVec[i].sizeMod;
        if (m_muzzleFlashVec[i].isFlickerTrue == true)
        {
            scale *= m_muzzleFlashVec[i].flickerScale;
            m_muzzleFlashVec[i].isFlickerTrue = false;
        }
        else
        {
            m_muzzleFlashVec[i].isFlickerTrue = true;
        }
        ...
        DirectX::SimpleMath::Vector3 posOffset = DirectX::SimpleMath::Vector3(0.0f, 0.5f, 0.0f);
        m_muzzleFlashVec[i].worldTestMatrix = DirectX::SimpleMath::Matrix::Identity;
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationY(testRot);
        m_muzzleFlashVec[i].worldTestMatrix *= scaleMat;
        m_muzzleFlashVec[i].worldTestMatrix *= scaleTransOffsetMat;
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(posOffset);
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(Utility::ToRadians(90.0f));
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(m_playerVehicle->GetWeaponPitch());
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationY(m_playerVehicle->GetTurretYaw());

        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetLocalizedMuzzlePos());
        m_muzzleFlashVec[i].worldTestMatrix *= updateMat;

        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix = DirectX::SimpleMath::Matrix::Identity;
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= scaleMat;
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= scaleTransOffsetMat;
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(posOffset);
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(DirectX::SimpleMath::Vector3(0.0f, (-0.5f * scale), 0.0f));
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(Utility::ToRadians(90.0f));
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(m_playerVehicle->GetWeaponPitch());
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateRotationY(m_playerVehicle->GetTurretYaw());
...
void FireControl::UpdateMuzzleFlashOld(MuzzleFlash& aMuzzleFlash, const double aTimeDelta)
{
    aMuzzleFlash.flashTimer += static_cast<float>(aTimeDelta);
    if (aMuzzleFlash.flashTimer >= aMuzzleFlash.flashDuration)
    {
...
void FireControl::UpdateMuzzleFlashVec(const double aTimeDelta)
{
    for (unsigned int i = 0; i < m_muzzleFlashVec.size(); ++i)
    {
        m_muzzleFlashVec[i].flashTimer += static_cast<float>(aTimeDelta);
        ...
        //m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(m_playerVehicle->GetWeaponPitch());
        m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateRotationY(m_playerVehicle->GetTurretYaw());
        m_muzzleFlashVec[i].worldTestMatrix *= m_muzzleFlashVec[i].localOrientation;
        //m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetLocalizedMuzzlePos());
        if (m_muzzleFlashVec[i].flashType == MuzzleFlashType::MUZZLEFLASHTYPE_LEFT)
        {
            m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetMissleTubePosLeftLocalized());
        }
        if (m_muzzleFlashVec[i].flashType == MuzzleFlashType::MUZZLEFLASHTYPE_RIGHT)
        {
            m_muzzleFlashVec[i].worldTestMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetMissleTubePosRightLocalized());
        }
        m_muzzleFlashVec[i].worldTestMatrix *= updateMat;
        ...
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateRotationZ(m_playerVehicle->GetWeaponPitch());
        //m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateRotationY(m_playerVehicle->GetTurretYaw());
        m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= m_muzzleFlashVec[i].localOrientation;
        //m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetLocalizedMuzzlePos());
        if (m_muzzleFlashVec[i].flashType == MuzzleFlashType::MUZZLEFLASHTYPE_LEFT)
        {
            m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetMissleTubePosLeftLocalized());
        }
        if (m_muzzleFlashVec[i].flashType == MuzzleFlashType::MUZZLEFLASHTYPE_RIGHT)
        {
            m_muzzleFlashVec[i].worldMuzzleFlashConeMatrix *= DirectX::SimpleMath::Matrix::CreateTranslation(m_playerVehicle->GetMissleTubePosRightLocalized());
        }
    ...
    for (unsigned int i = 0; i < m_muzzleFlashVec.size(); ++i)
    {
        if (m_muzzleFlashVec[i].isDeleteTrue == true)
        {
            std::vector<MuzzleFlash>::iterator it;
            it = m_muzzleFlashVec.begin() + i;

            m_muzzleFlashVec.erase(it);
        }
    }
}
...
DirectX::SimpleMath::Vector3 FireControl::Pursuit(MissileData& aMissile, const DirectX::SimpleMath::Vector3 aVelLocal, const DirectX::SimpleMath::Vector3 aTargLocalPos, const DirectX::SimpleMath::Vector3 aTargLocalVel)
{
    ...
    auto toEvader = aMissile.guidance.targetPosLocal;
    auto toEvaderNorm = toEvader;
    toEvaderNorm.Normalize();

    auto selfHeadingNormLocal = aVelLocal;
    selfHeadingNormLocal.Normalize();
    auto targetHeadingNormLocal = aTargLocalVel;
    targetHeadingNormLocal.Normalize();
    //float relativeHeading = aVelLocal.Dot(aTargLocalVel);
    float relativeHeading = selfHeadingNormLocal.Dot(targetHeadingNormLocal);

    m_debugData->ToggleDebugOnOverRide();
    m_debugData->DebugPushUILineDecimalNumber("relativeHeading = ", relativeHeading, "");

    m_debugData->ToggleDebugOff();

    //if (toEvader.Dot(aVelLocal) > 0.0f && (relativeHeading < -0.95f)) //acos(0.95)=18 degs
    if (toEvaderNorm.Dot(selfHeadingNormLocal) > 0.0f && (relativeHeading < -0.95f)) //acos(0.95)=18 degs
    {
        return Seek(aVelLocal, aTargLocalPos);
    }

    //float lookAheadTime = toEvader.Length() / (m_missileConsts.velMaxEst + aTargLocalVel.Length());
    float lookAheadTime = toEvader.Length() / (m_missileConsts.velMaxEst + aMissile.guidance.targetVelLocal.Length());
    //lookAheadTime = 1.0f;
    ...
    return Seek(aMissile.guidance.selfVelLocal, aMissile.guidance.targetPosLocal + aMissile.guidance.targetVelLocal * lookAheadTime);
}
...
void FireControl::UpdateFlightDataDependantVars(MissileData& aMissile, const double aTimeDelta)
{
    ...
    if (aMissile.projectileData.time >= m_missileConsts.rocketFireFullTime + m_missileConsts.rocketFireDelay)
    {
        ...
        */
        ...
        else if (speed > m_missileConsts.contrailVelocityMax)
        {
            aMissile.guidance.contrailLength = m_ammoMissile.modelData.contrailDrawCountMax;
        }
        else
        {
    ...
    else
    {
        ...
        DirectX::SimpleMath::Vector3 centerOfPressure = m_missileConsts.centerOfPressureBasePosLocal;
        centerOfPressure += m_missileConsts.centerOfPressureFullFinDeployOffset * aMissile.guidance.finDeployPercent;
        centerOfPressure += m_missileConsts.centerOfPressureAngleOfAttackOffSetMax * abs(aMissile.guidance.angleOfAttack / Utility::ToRadians(90.0f));
        aMissile.guidance.centerOfPressureLocalPos = centerOfPressure;
    }
}
...
void FireControl::UpdateFlightDataIndependentVars(MissileData& aMissile, const double aTimeDelta)
{
    ...
    targPosUpNorm = DirectX::SimpleMath::Vector3::Transform(targPosUpNorm, toUpQuat);
    //auto targPosUpCord = targPosL;
    auto targPosUpCord = aMissile.guidance.targetPosLocal;
    targPosUpCord.Normalize();
    //targPosUpCord = DirectX::SimpleMath::Vector3::Transform(targPosUpCord, toUpQuat);
    targPosUpCord = DirectX::SimpleMath::Vector3::Transform(targPosUpCord, invsToUpQuat);
    targPosUpCord = DirectX::SimpleMath::Vector3::Transform(targPosUpCord, aMissile.projectileData.alignmentQuat);
    ...
    {
        isTargetBehindTrue = true;
        aMissile.guidance.isFacingDestTrue = false;
    }
    else
    {
        isTargetBehindTrue = false;
...
void FireControl::UpdateFlightState(MissileData& aMissile, const double aTimeDelta)
{
    ...
    if (aMissile.guidance.flightStateCurrent == FlightState::FLIGHTSTATE_CRUISE)
    {
        aMissile.guidance.testTimer += aTimeDelta;

        float velocity = aMissile.projectileData.q.velocity.Length();

        if (aMissile.guidance.testTimerTriggerIsTrueA == true)
        {
            aMissile.guidance.testTimerTriggerIsTrueB = true;
        }

        if (aMissile.guidance.testTimerTriggerIsTrueA == false)
        {
            aMissile.guidance.testTimerTriggerIsTrueA = true;
        }
        ...
        if (aMissile.guidance.isFacingDestTrue == false)
        {
            aMissile.guidance.flightStateCurrent = FlightState::FLIGHTSTATE_OVERSHOOT;
        }
    }

    if (aMissile.guidance.flightStateCurrent == FlightState::FLIGHTSTATE_OVERSHOOT)
    {

        float deltaA = aMissile.guidance.testTimerFirstGuessA - aMissile.guidance.testTimer;
        float deltaB = aMissile.guidance.testTimerFirstGuessB - aMissile.guidance.testTimer;
...
void FireControl::UpdateFlightDataTarget(MissileData& aMissile, const double aTimeDelta)
{
    ...
    auto selfVelLocalNorm = selfVelL;
    selfVelLocalNorm.Normalize();
    const float velNormSize = selfVelLocalNorm.Length();

    if (velNormSize < 0.9 || velNormSize > 1.1f);
    {
        selfVelLocalNorm = DirectX::SimpleMath::Vector3::UnitX;
    }
    ...
    auto upLocal = DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3::UnitY, aMissile.projectileData.inverseAlignmentQuat);
    aMissile.guidance.selfUpLocal = upLocal;
}
...
void FireControl::GuidanceBasicGravity(MissileData& aMissile, const float aTimeDelta)
{
    ...
    auto interceptPosLocal = GuidanceCalcInercept(aMissile, aTimeDelta);
    ...
    */
    ...
    //auto targVecNorm = targPosL;
    auto targVecNorm = updateTargPosL;
    //auto targVecNorm = interceptPosLocal;
    targVecNorm.Normalize();
  ...
  //  m_debugData->ToggleDebugOnOverRide();
    ...
    if (aMissile.guidance.testTimerTriggerIsTrueA == true && aMissile.guidance.testTimerTriggerIsTrueB == false)
    {
        aMissile.guidance.testTimerFirstGuessB = timeToGo2;
    }
    
    float deltaB = (aMissile.guidance.testTimerDeltaB - timeToGo2) * aTimeDelta;
    aMissile.guidance.testTimerDeltaB = deltaB;
...
DirectX::SimpleMath::Vector3 FireControl::GuidanceCalcInercept(MissileData& aMissile, const float aTimeDelta)
{
    ...
    timeToCollision += a / aMissile.projectileData.q.velocity.Length();
    auto timeToCollisionA = a / aMissile.projectileData.q.velocity.Length();

    //float height = 0.5f * -m_environment->GetGravity() * (timeToImpact * timeToImpact);
    //float height = 0.5f * -m_environment->GetGravity() * ((timeToImpact * timeToImpact) * 0.5f);
    auto heightDropForTime = 0.5f * -m_environment->GetGravity() * (timeToCollision * timeToCollision);

    auto collisionPos = aMissile.guidance.targetPosition + (aMissile.guidance.targetVelocity * timeToCollision);
    auto collisionPosWithHeightDrop = collisionPos;
    collisionPosWithHeightDrop.y += heightDropForTime;

    auto intercepPos = aMissile.projectileData.q.position - collisionPos;

    auto collisionPosLocal = collisionPos - aMissile.projectileData.q.position;
    //auto collisionPosLocal = collisionPosWithHeightDrop - aMissile.projectileData.q.position;
    collisionPosLocal = DirectX::SimpleMath::Vector3::Transform(collisionPosLocal, aMissile.projectileData.inverseAlignmentQuat);

    DebugPushDrawData(DirectX::SimpleMath::Vector3::Zero, collisionPosLocal, DirectX::Colors::Pink, false, true);


    m_debugData->ToggleDebugOnOverRide();
    m_debugData->DebugPushUILineDecimalNumber("timeToCollision       = ", timeToCollision, "");
    m_debugData->DebugPushUILineDecimalNumber("timeToCollisionA      = ", timeToCollisionA, "");
    m_debugData->DebugPushUILineDecimalNumber("heightDropForTime       = ", heightDropForTime, "");

    m_debugData->PushDebugLinePositionIndicator(collisionPos, 20.0f, 0.0f, DirectX::Colors::White);
    //m_debugData->PushTestDebugBetweenPoints(aMissile.projectileData.q.position, intercepPos, DirectX::Colors::Orange);
    m_debugData->PushTestDebugBetweenPoints(aMissile.projectileData.q.position, collisionPos, DirectX::Colors::Blue);

    m_debugData->PushDebugLinePositionIndicator(collisionPosWithHeightDrop, 20.0f, 0.0f, DirectX::Colors::Gold);
    //m_debugData->PushTestDebugBetweenPoints(aMissile.projectileData.q.position, intercepPos, DirectX::Colors::Orange);
    m_debugData->PushTestDebugBetweenPoints(aMissile.projectileData.q.position, collisionPosWithHeightDrop, DirectX::Colors::Teal);

    m_debugData->ToggleDebugOff();

    if (aMissile.guidance.testTimerTriggerIsTrueA == true && aMissile.guidance.testTimerTriggerIsTrueB == false)
    {
        aMissile.guidance.testTimerFirstGuessA = timeToCollision;
    }

    float deltaA = (aMissile.guidance.testTimerDeltaA - timeToCollision) * aTimeDelta;
    aMissile.guidance.testTimerDeltaA = deltaA;

    //return aMissile.projectileData.q.position - collisionPos;
    return collisionPosLocal;
}
...
void FireControl::GuidancePrototype(MissileData& aMissile, const float aTimeDelta)
{
    ...
    auto timeToTargetDirect = distance / selfSpeed;
    ...
    {
        timeToTargetDirect = timeToTargetMax;
    }
   
    auto targVelNormW = aMissile.guidance.targetVelocity;
    targVelNormW.Normalize();

    auto targPosAtTime = (targVelNormW * aMissile.guidance.targetVelocity.Length()) * timeToTargetDirect;
    targPosAtTime += aMissile.guidance.targetPosition;

    auto gravDrop = 0.5f * m_environment->GetGravity() * (timeToTargetDirect * timeToTargetDirect);
    targPosAtTime.y -= gravDrop;

    //m_debugData->PushDebugLinePositionIndicator(targPosAtTime, 20.0f, 0.0f, DirectX::Colors::Coral);
    //m_debugData->PushTestDebugBetweenPoints(aMissile.projectileData.q.position, targPosAtTime, DirectX::Colors::Lime);
    m_debugData->DebugPushUILineDecimalNumber("gravDrop = ", gravDrop, "");
    m_debugData->DebugPushUILineDecimalNumber("timeToTargetDirect = ", timeToTargetDirect, "");

    m_debugData->ToggleDebugOff();

    auto targPosAtTimeLocal = targPosAtTime;
    targPosAtTimeLocal -= aMissile.projectileData.q.position;
    targPosAtTimeLocal = DirectX::SimpleMath::Vector3::Transform(targPosAtTimeLocal, aMissile.projectileData.inverseAlignmentQuat);

    //auto targVecNorm = aMissile.guidance.targetPosLocal;
    auto targVecNorm = targPosAtTimeLocal;
    targVecNorm.Normalize();
...
void FireControl::GuidancePrototypeOld(MissileData& aMissile, const float aTimeDelta)
{
    //m_debugData->ToggleDebugOnOverRide();
...
void FireControl::FireSelectedWithAudio(const DirectX::SimpleMath::Vector3 aLaunchPos, const DirectX::SimpleMath::Vector3 aLaunchDirectionForward, const DirectX::SimpleMath::Vector3 aLauncherVelocity, const DirectX::SimpleMath::Vector3 aUp, std::shared_ptr<Utility::SoundFx> aFireFx, bool aDebugToggle)
{
    if (m_isCoolDownActive == false)
    {
