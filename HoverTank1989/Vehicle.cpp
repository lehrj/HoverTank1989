...
{
    DirectX::SimpleMath::Matrix rotMat = DirectX::SimpleMath::Matrix::CreateFromAxisAngle(DirectX::SimpleMath::Vector3::UnitY, Utility::ToRadians(90.0f));
    m_heli.alignment *= rotMat;
    m_heli.q.alignmentQuat *= DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(rotMat);
    //testAngularPos = (m_heli.q.angularPosQuat * m_heli.q.alignmentQuat
...
void Vehicle::InitializeVehicle(Microsoft::WRL::ComPtr<ID3D11DeviceContext1> aContext, std::shared_ptr<NPCController> aNPCController, std::shared_ptr<Vehicle> aVehicle)
{
    ...
    m_heli.q.angularPosQuat = DirectX::SimpleMath::Quaternion::Identity;
    m_heli.q.alignmentQuat = DirectX::SimpleMath::Quaternion::Identity;
    m_heli.q.inverseAlignmentQuat = m_heli.q.alignmentQuat;
    m_heli.q.inverseAlignmentQuat.Inverse(m_heli.q.inverseAlignmentQuat);
    m_heli.q.inverseAlignmentQuat.Normalize();
    ...
    DirectX::SimpleMath::Matrix testMat = m_heli.inertiaMatrixTest;
    ...
    float radius = 4.0f;
    //sphere
    DirectX::SimpleMath::Matrix flippedMat = DirectX::SimpleMath::Matrix::Identity;
    ...
    m_testInertiaFloat = m_testInertiaTensorLocal._11;
    m_testInverseInertiaFloat = m_testInverseInertiaTensorLocal._11;
    ...
    m_testTensor._11 = m_heli.toUseTensor._11;
    m_testTensor._12 = m_heli.toUseTensor._12;
    m_testTensor._13 = m_heli.toUseTensor._13;
    m_testTensor._21 = m_heli.toUseTensor._21;
    m_testTensor._22 = m_heli.toUseTensor._22;
    m_testTensor._23 = m_heli.toUseTensor._23;
    m_testTensor._31 = m_heli.toUseTensor._31;
    m_testTensor._32 = m_heli.toUseTensor._32;
    m_testTensor._33 = m_heli.toUseTensor._33;


    //= m_heli.toUseTensor;

    DirectX::SimpleMath::Matrix testMat2 = m_heli.toUseTensor;
    //m_heli.toUseTensor._44 = 1.0f;
    DirectX::SimpleMath::Matrix testMat4 = m_heli.toUseTensor;
...
void Vehicle::RightHandSide(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    ...
    newQ.angularPosQuat = aQ->angularPosQuat + static_cast<float>(aQScale) * aDeltaQ->angularPosQuat;

    newQ.gOrientationQuat = aQ->gOrientationQuat + static_cast<float>(aQScale) * aDeltaQ->gOrientationQuat;
    newQ.gAngularMomentum = aQ->gAngularMomentum + static_cast<float>(aQScale) * aDeltaQ->gAngularMomentum;
    newQ.gSpin = aQ->gSpin + static_cast<float>(aQScale) * aDeltaQ->gSpin;
    newQ.gAngularVelocity = aQ->gAngularVelocity + static_cast<float>(aQScale) * aDeltaQ->gAngularVelocity;
    newQ.gTestOrientation = aQ->gTestOrientation + static_cast<float>(aQScale) * aDeltaQ->gTestOrientation;

    newQ.toUseOrientation = aQ->toUseOrientation + static_cast<float>(aQScale) * aDeltaQ->toUseOrientation;
    newQ.toUseSpin = aQ->toUseSpin + static_cast<float>(aQScale) * aDeltaQ->toUseSpin;
    newQ.toUseAngularMomentum = aQ->toUseAngularMomentum + static_cast<float>(aQScale) * aDeltaQ->toUseAngularMomentum;
    newQ.toUseAngularVelocity = aQ->toUseAngularVelocity + static_cast<float>(aQScale) * aDeltaQ->toUseAngularVelocity;

    newQ.angularVelocityUpdateTest = aQ->angularVelocityUpdateTest + static_cast<float>(aQScale) * aDeltaQ->angularVelocityUpdateTest;
    newQ.angularMomentumUpdateTest = aQ->angularMomentumUpdateTest + static_cast<float>(aQScale) * aDeltaQ->angularMomentumUpdateTest;
    ...
    DirectX::SimpleMath::Vector3 accelVecUpdate = m_heli.q.angPosVec;
    ...
    //DirectX::SimpleMath::Vector3 angDampTest2 = newQ.angPosVec * m_angularDampConst;
    DirectX::SimpleMath::Vector3 angDampTest2 = m_heli.q.angularVelocityVec * m_angularDampConst;
    //DirectX::SimpleMath::Vector3 angDampTest2 = newQ.angularVelocityVec * m_angularDampConst;
    //DirectX::SimpleMath::Vector3 angDampTest3 = newQ.angPosVec * m_angularDampConst;
    //accelVecUpdate += angDampTest;
    if (m_debugToggle3 == false)
    {
        //accelVecUpdate -= angDampTest2;
    }
    ...
    DirectX::SimpleMath::Quaternion updateVelocityQuat = DirectX::SimpleMath::Quaternion::Identity;
    updateVelocityQuat = m_heli.q.alignmentQuat;
    //updateVelocityQuat = m_heli.q.angularVelocityQuat;
    //updateVelocityQuat = m_heli.q.angularPosQuat;
    updateVelocityQuat = newQ.angularPosQuat;
    if (accelVecUpdate.Length() != 0.0f)
    {
        updateVelocityQuat *= DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(accelVecUpdate, accelVecUpdate.Length());
    }

    DirectX::SimpleMath::Quaternion updatePosQuat = DirectX::SimpleMath::Quaternion::Identity;
    //updatePosQuat = m_heli.q.angularPosQuat;
    //updatePosQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
    //if (newQ.angularVelocityVec.Length() != 0.0f)
    if (newQ.angPosVec.Length() != 0.0f)
    {
        //updatePosQuat *= DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(newQ.angularVelocityVec, newQ.angularVelocityVec.Length());
        updatePosQuat *= DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(newQ.angPosVec, newQ.angPosVec.Length());
    }
    aDQ->angularVelocityQuat = static_cast<float>(aTimeDelta) * updateVelocityQuat;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
    /*
    DirectX::SimpleMath::Quaternion toUseOrientationWorking = DirectX::SimpleMath::Quaternion::Identity;
    DirectX::SimpleMath::Quaternion toUseSpinWorking = DirectX::SimpleMath::Quaternion::Identity;
    DirectX::SimpleMath::Quaternion toMakeOrientation = DirectX::SimpleMath::Quaternion::Identity;
    DirectX::SimpleMath::Vector3 toUseVelocityAngWorking = DirectX::SimpleMath::Vector3::Zero;
    DirectX::SimpleMath::Vector3 toUseMomentumAngWorking = DirectX::SimpleMath::Vector3::Zero;
    */
    DirectX::SimpleMath::Quaternion toUseOrientationWorking = newQ.toUseOrientation;
    DirectX::SimpleMath::Quaternion toUseSpinWorking = newQ.toUseSpin;
    DirectX::SimpleMath::Quaternion toMakeOrientation = newQ.toUseOrientation;
    DirectX::SimpleMath::Vector3 toUseVelocityAngWorking = newQ.toUseAngularVelocity;
    DirectX::SimpleMath::Vector3 toUseMomentumAngWorking = newQ.toUseAngularMomentum;

    // prev declared, will need to declare after clean up
    //DirectX::SimpleMath::Vector3 vAngularVelocity = DirectX::SimpleMath::Vector3::Zero;
    //DirectX::SimpleMath::Vector3 momentsInput = DirectX::SimpleMath::Vector3::Zero;
    //DirectX::SimpleMath::Vector3 toCrossVelocity = DirectX::SimpleMath::Vector3::Zero;
    //DirectX::SimpleMath::Vector3 toInverseInertiaVelocity = DirectX::SimpleMath::Vector3::Zero;

    DirectX::SimpleMath::Quaternion toUseOrientationUpdate = DirectX::SimpleMath::Quaternion::Identity;
    DirectX::SimpleMath::Quaternion toUseSpinUpdate = DirectX::SimpleMath::Quaternion::Identity;
    DirectX::SimpleMath::Vector3 toUseVelocityAngUpdate = DirectX::SimpleMath::Vector3::Zero;
    DirectX::SimpleMath::Vector3 toUseMomentumAngUpdate = DirectX::SimpleMath::Vector3::Zero;
    toUseOrientationWorking = m_heli.q.toUseOrientation;
    toUseMomentumAngUpdate = toUseMomentumAngWorking;
    toUseMomentumAngUpdate = m_heli.q.toUseAngularMomentum;
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//    DirectX::SimpleMath::Vector3 vAngularVelocity = m_angVel;
    DirectX::SimpleMath::Vector3 vAngularVelocity = toUseVelocityAngWorking;
//    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, m_testOrientQuat);
    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, toUseOrientationWorking);
    DirectX::SimpleMath::Vector3 testInput = DirectX::SimpleMath::Vector3::Zero;
    testInput.x = -m_heli.controlInput.cyclicInputRoll;
    testInput.y = -m_heli.controlInput.yawPedalInput;
    testInput.z = -m_heli.controlInput.cyclicInputPitch;
    /*
    testInput = DirectX::SimpleMath::Vector3(1.0f, 0.0f, 0.0f);
    if (m_debugToggle == true)
    {
        testInput = DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f);
    }
    ...
    testInput = DirectX::SimpleMath::Vector3(-2.0f, 0.0f, 0.0f);
    if (m_debugToggle == true)
    {
        testInput = DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f);
    }
    /*
    testInput *= aTimeDelta;
    //testInput += m_prevInput;
 //   testInput += m_angMom;
    testInput += toUseMomentumAngUpdate;
    
    //m_prevInput = testInput;
 //   m_angMom = testInput;
    toUseMomentumAngUpdate = testInput;
    testInput *= aTimeDelta;
    */
 ...
 //   testInput += m_angMom;
    //testInput += toUseMomentumAngUpdate;
    DirectX::SimpleMath::Vector3 addToInputMom = (toUseMomentumAngUpdate * aTimeDelta);
    testInput += addToInputMom;
    //m_prevInput = testInput;
 //   m_angMom = testInput;
    toUseMomentumAngUpdate = testInput;
    //testInput *= aTimeDelta;
    */
    ...
    testInputScaled = testInput;
    testInput = newQ.toUseAngularMomentum;

    DirectX::SimpleMath::Vector3 angDragTest = testInput;
    DirectX::SimpleMath::Vector3 angDrag = CalculateDragAngularLocal(angDragTest);
    angDrag *= 0.001f;

    testInput += angDrag;

    testInput += testInputScaled;

    //testInput = DirectX::SimpleMath::Vector3(1.0f, 0.0f, 0.0f);
    toUseMomentumAngUpdate = newQ.toUseAngularMomentum * aTimeDelta;
    //toUseMomentumAngUpdate = DirectX::SimpleMath::Vector3::Zero;
    //testInput *= aTimeDelta;
    //testInput += m_prevInput;
 //   testInput += m_angMom;
    //testInput += toUseMomentumAngUpdate;
    //DirectX::SimpleMath::Vector3 addToInputMom = (toUseMomentumAngUpdate * aTimeDelta);
    DirectX::SimpleMath::Vector3 addToInputMom = (newQ.toUseAngularMomentum * aTimeDelta);
    //testInput += addToInputMom;
    //m_prevInput = testInput;
 //   m_angMom = testInput;
    toUseMomentumAngUpdate += testInput;
            //testInput += addToInputMom;
    //testInput *= aTimeDelta;
    //toUseMomentumAngUpdate *= 0.5f;
    DirectX::SimpleMath::Vector3 momentsInput = testInput;

    DirectX::SimpleMath::Vector3 momUpdate = toUseMomentumAngUpdate;
    //toUseMomentumAngUpdate += testInput;
            //testInput += newQ.toUseAngularMomentum;
    //m_prevInput = testInput;
    ...
    //m_prevInput2 = vAngularVelocity;
//    m_angVel = vAngularVelocity;
    toUseVelocityAngUpdate = vAngularVelocity;
    
    //m_testOrientQuat.Normalize();
    toUseOrientationWorking.Normalize();
    DirectX::SimpleMath::Quaternion testInputQuat;
    //testInput = DirectX::SimpleMath::Vector3::Transform(testInput, m_testOrientQuat);
    //m_prevInput = testInput;
    //testInput = vAngularVelocity;
    //momUpdate += vAngularVelocity;
    //testInput = momentsInput;
    momUpdate = testInput;
    if (m_debugToggle == true)
    {
        //testInput = DirectX::SimpleMath::Vector3::Zero;
    }
    testInputQuat.x = testInput.x;
    testInputQuat.y = testInput.y;
    testInputQuat.z = testInput.z;
    testInputQuat.w = 0.0f;
    //DirectX::SimpleMath::Quaternion testAngularPos = 0.5f * testInputQuat * (m_testQuat * m_testOrientQuat);
//    DirectX::SimpleMath::Quaternion testAngularPos = m_testOrientQuat;
//    testAngularPos += (0.5f * aTimeDelta) * (testInputQuat * m_testOrientQuat);
    DirectX::SimpleMath::Quaternion testAngularPos = toUseOrientationWorking;
    DirectX::SimpleMath::Quaternion preTestQuat = testAngularPos;
    //testAngularPos += (0.5f * aTimeDelta) * (testInputQuat * toUseOrientationWorking);
    testAngularPos += (aTimeDelta) * (testInputQuat * toUseOrientationWorking);
    testAngularPos.Normalize();
   
    DirectX::SimpleMath::Quaternion postTestQuat = testAngularPos;
    //DirectX::SimpleMath::Quaternion deltaTestQuat = preTestQuat - postTestQuat;
    DirectX::SimpleMath::Quaternion deltaTestQuat = postTestQuat - preTestQuat;
    DirectX::SimpleMath::Quaternion deltaTestQuatNorm = deltaTestQuat;
    deltaTestQuatNorm.Normalize();
    /*
    testAngularPos = toUseOrientationWorking;
    Utility::AddScaledVectorToQuat(testInput, aTimeDelta, testAngularPos);
    testAngularPos.Normalize();
    */

    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    DirectX::SimpleMath::Vector3 momentumUpdate = newQ.angularMomentumUpdateTest;
    //momentumUpdate *= aTimeDelta;
    ...
    testInput.z += -m_heli.controlInput.cyclicInputPitch;
    //testInput.x *= 10.0f;
    //testInput = DirectX::SimpleMath::Vector3(100.0f, 0.0f, 0.0f);
    //testInput = m_testTorqueLocal;
    ...
    testInputScaled = testInput * aTimeDelta;

    //////
    /*
    momentsInput = momentumUpdate;
    vAngularVelocity = testInput;
    vAngularVelocity = torqueForce;

    toCrossVelocity = vAngularVelocity;
    toCrossVelocity = DirectX::SimpleMath::Vector3::Transform(toCrossVelocity, m_testInertiaTensorLocal);

    toInverseInertiaVelocity = vAngularVelocity;
    ...
    */
    ...
    DirectX::SimpleMath::Vector3 AirplanevAngularVelocity = newQ.angularVelocityUpdateTest;
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //aDQ->toUseOrientation = static_cast<float>(aTimeDelta) * toUseOrientationUpdate;
    //aDQ->toUseOrientation = static_cast<float>(aTimeDelta) * toMakeOrientation;
    aDQ->toUseOrientation = static_cast<float>(aTimeDelta) * testAngularPos;
    aDQ->toUseSpin = static_cast<float>(aTimeDelta) * toUseSpinUpdate;
    aDQ->toUseAngularMomentum = static_cast<float>(aTimeDelta) * toUseMomentumAngUpdate;
    aDQ->toUseAngularMomentum = static_cast<float>(aTimeDelta) * momUpdate;
    //aDQ->toUseAngularMomentum = toUseMomentumAngUpdate;
    aDQ->toUseAngularVelocity = static_cast<float>(aTimeDelta) * toUseVelocityAngUpdate;

    //aDQ->angularVelocityUpdateTest = static_cast<float>(aTimeDelta) * testInput;
    //aDQ->angularVelocityUpdateTest = static_cast<float>(aTimeDelta) * newQ.angularVelocityUpdateTest;
    //aDQ->angularVelocityUpdateTest = static_cast<float>(aTimeDelta) * newQ.angularMomentumUpdateTest;
    aDQ->angularVelocityUpdateTest = static_cast<float>(aTimeDelta) * updatedAngularVelocity;
    //aDQ->angularMomentumUpdateTest = static_cast<float>(aTimeDelta) * momentumUpdate;
    //aDQ->angularMomentumUpdateTest = static_cast<float>(aTimeDelta) * testInput;
    aDQ->angularMomentumUpdateTest = static_cast<float>(aTimeDelta) * torqueForce;
}
...
void Vehicle::RightHandSide3(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    //  Compute the intermediate values of the 
    //  dependent variables.
    Motion newQ;
    ...
    if (m_debugToggle5 == true)
    {
        float angRate = Utility::ToRadians(3.0f);

        angRate *= static_cast<float>(aTimeDelta);
        DirectX::SimpleMath::Vector3 angAxis = DirectX::SimpleMath::Vector3::UnitX;

        DirectX::SimpleMath::Quaternion angQuat = DirectX::SimpleMath::Quaternion::Identity;
        angQuat = aHeli->q.angularPosQuat;
        //angQuat = aHeli->q.angularVelocityQuat;
        //angQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
        angQuat = DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(angAxis, angRate);
        updateVelocityQuat = angQuat;
    }
    ...
    aDQ->angularVelocityQuat = static_cast<float>(aTimeDelta) * updateVelocityQuat;
    //aDQ->angularVelocityQuat = static_cast<float>(aTimeDelta) * newQ.angularVelocityQuat;
    //aDQ->angularQuat = static_cast<float>(aTimeDelta) * (newQ.angularVelocityVec);
    //aDQ->angularQuat = DirectX::SimpleMath::Quaternion::Identity;
    //aDQ->angularPosQuat = static_cast<float>(aTimeDelta) * updatePosQuat;
    aDQ->angularPosQuat = static_cast<float>(aTimeDelta) * newQ.angularVelocityQuat;
    //aDQ->angularPosQuat.Normalize();
    //aDQ->bodyTorqueForce = angAccelVecTensorUpdate;
}

void Vehicle::RightHandSideLastUsed(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    //  Compute the intermediate values of the 
    //  dependent variables.
    Motion newQ;
    ...
    //DirectX::SimpleMath::Quaternion toUseQuat = newQ.gOrientationQuat;
    DirectX::SimpleMath::Quaternion toUseQuat = m_heli.q.gSpin;
    //DirectX::SimpleMath::Vector3 toUseVec = newQ.gAngularMomentum;
    DirectX::SimpleMath::Vector3 toUseVec = newQ.gAngularMomentum;
    DirectX::SimpleMath::Vector3 testNorm = toUseVec;
    testNorm.Normalize();
    testNorm = DirectX::SimpleMath::Vector3::Transform(testNorm, toUseQuat);
    testNorm *= toUseQuat.Length();
    //angularMomentum = testNorm;
    ...
    if (m_debugToggle9 == false)
    {
        ...
        DirectX::SimpleMath::Vector3 momUpdate = DirectX::SimpleMath::Vector3(1.0f, 0.0f, 0.0f);
    ...
    if (m_debugToggle8 == true)
    {
        ...
        angularMomentum = angularMomentum * -0.3f;
    }

    angularMomentum = m_heli.q.gAngularVelocity;
    //angularMomentum = m_heli.q.gAngularMomentum;
    //angularMomentum += m_heli.vehicleAngularForcesSum;
    angularMomentum += m_testTorqueLocal;
    //angularMomentum = m_testTorqueWorld;
    ...
    //Airplane.vAngularVelocity += Airplane.mInertiaInverse * (Airplane.vMoments - (Airplane.vAngularVelocity ^ (Airplane.mInertia * Airplane.vAngularVelocity))) * dt;
    DirectX::SimpleMath::Vector3 prevAngularVelocity = m_heli.q.gAngularVelocity;
    DirectX::SimpleMath::Vector3 vAngularVelocity = m_heli.q.gAngularVelocity;
    DirectX::SimpleMath::Vector3 vAngularMomentum = m_heli.q.gAngularMomentum;
    DirectX::SimpleMath::Vector3 momentsInput = m_testTorqueLocal;
    momentsInput = DirectX::SimpleMath::Vector3(0.0f, 0.0f, 0.0f);
    //momentsInput = DirectX::SimpleMath::Vector3(10.0f, 0.0f, 0.0f) - (vAngularVelocity * 0.9f);
    if (m_debugToggle9 == false)
    {
        momentsInput = DirectX::SimpleMath::Vector3(1.0f, 0.0f, 0.0f);
    }
    if (m_debugToggle8 == true)
    {
        int testBreak = 0;
    ...
    DirectX::SimpleMath::Vector3 preVelocity = vAngularVelocity;
    DirectX::SimpleMath::Vector3 expVelocity = vAngularVelocity;
    ////////////expVelocity = DirectX::SimpleMath::Vector3::Transform(expVelocity, m_testInertiaTensorLocal);
    expVelocity = expVelocity * m_testInertiaFloat;

    DirectX::SimpleMath::Vector3 toCrossVelocity = vAngularVelocity;
    toCrossVelocity = DirectX::SimpleMath::Vector3::Transform(toCrossVelocity, m_testInertiaTensorLocal);
    ////////////toCrossVelocity = toCrossVelocity * m_testInertiaFloat;
    DirectX::SimpleMath::Vector3 toInverseInertiaVelocity = vAngularVelocity;
    toInverseInertiaVelocity = (momentsInput - (vAngularVelocity.Cross(toCrossVelocity)));
    //vAngularVelocity += m_testInverseInertiaTensorLocal * (momentsInput - (vAngularVelocity ^ (m_testInertiaTensorLocal * vAngularVelocity)));
    //vAngularVelocity += m_testInverseInertiaTensorLocal * (momentsInput - (vAngularVelocity ^ (expVelocity)));
    //vAngularVelocity += m_testInverseInertiaTensorLocal * (momentsInput - (vAngularVelocity.Cross(toCrossVelocity)));
    toInverseInertiaVelocity = DirectX::SimpleMath::Vector3::Transform(toInverseInertiaVelocity, m_testInverseInertiaTensorLocal);
    ////////toInverseInertiaVelocity = toInverseInertiaVelocity * m_testInverseInertiaFloat;
    //toInverseInertiaVelocity *= static_cast<float>(aTimeDelta);
    vAngularVelocity = toInverseInertiaVelocity;
    DirectX::SimpleMath::Vector3 postvAngularVelocity = vAngularVelocity;

    //vAngularVelocity *= static_cast<float>(aTimeDelta);

    DirectX::SimpleMath::Vector3 toQVec = preVelocity * static_cast<float>(aTimeDelta);
    toQVec += toInverseInertiaVelocity * static_cast<float>(aTimeDelta);
    ...
    //q.Normalize();
    //DirectX::SimpleMath::Quaternion spinUpdate = 0.5f * q * orientation;
    //DirectX::SimpleMath::Quaternion qOrientation = m_heli.q.gTestOrientation;
    DirectX::SimpleMath::Quaternion qOrientation = newQ.gTestOrientation;
    qOrientation.Normalize();
    //qOrientation += (qOrientation * q) * (0.5f * static_cast<float>(aTimeDelta));
    //qOrientation = (qOrientation * q) * (0.5f);
    //qOrientation = 0.5f * q * qOrientation;
    DirectX::SimpleMath::Quaternion qSpin = 0.5f * q * qOrientation;
    //qOrientation.Normalize();
    ...
    aDQ->gAngularMomentum = static_cast<float>(aTimeDelta) * momentsInput;
    ...
    //aDQ->gAngularVelocity = vAngularVelocity;
    aDQ->gAngularVelocity = static_cast<float>(aTimeDelta) * vAngularVelocity;
    //aDQ->gTestOrientation = static_cast<float>(aTimeDelta) * DirectX::SimpleMath::Quaternion::Identity;
    //aDQ->gTestOrientation = static_cast<float>(aTimeDelta) * newQ.gSpin;
    //aDQ->gTestOrientation = static_cast<float>(aTimeDelta) * newQ.gOrientationQuat;
    //aDQ->gTestOrientation = qOrientation;
    aDQ->gTestOrientation = static_cast<float>(aTimeDelta) * qSpin;
}

void Vehicle::RightHandSide4(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    //  Compute the intermediate values of the 
    //  dependent variables.
    Motion newQ;
    ...
    DirectX::SimpleMath::Vector3 prevInput = newQ.toUseAngularVelocity;
    DirectX::SimpleMath::Vector3 prevInput2 = newQ.toUseAngularMomentum;
    DirectX::SimpleMath::Vector3 passPrevInput;
    DirectX::SimpleMath::Vector3 passPrevInput2;

    //vAngularVelocity = m_prevInput2;
    vAngularVelocity = prevInput2;
    //vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, m_testOrientQuat);
    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, newQ.toUseOrientation);
    ...
    testInput += prevInput;
    //m_prevInput = testInput;
    passPrevInput = testInput;
    //testInput *= aTimeDelta;
    momentsInput = testInput;

    //////////////////////////////////////////////////////////////////////////////////
    vAngularVelocity = newQ.toUseAngularVelocity;
    toCrossVelocity = vAngularVelocity; ///?????????
    toCrossVelocity = DirectX::SimpleMath::Vector3::Transform(toCrossVelocity, m_testInertiaTensorLocal);
    ...
    vAngularVelocity = toInverseInertiaVelocity;
    //m_prevInput2 = vAngularVelocity;
    passPrevInput2 = vAngularVelocity;
    //////////////////////////////////////////////////////////////////////////////////
    //m_testOrientQuat.Normalize();
    newQ.toUseOrientation.Normalize();
    DirectX::SimpleMath::Quaternion testInputQuat;
    ...
    //testAngularPos = 0.5f * (testInputQuat) * (m_testOrientQuat);
    //testAngularPos += m_testOrientQuat;
    //testAngularPos.Normalize();

    newQ.toUseOrientation;
    //toMakeOrientation = 0.5f * (testInputQuat) * (m_testOrientQuat);
    //toMakeOrientation += m_testOrientQuat;
    toMakeOrientation = 0.5f * (testInputQuat) * (newQ.toUseOrientation);
    toMakeOrientation += newQ.toUseOrientation;
    toMakeOrientation.Normalize();
    ...
    toUseVelocityAngUpdate = passPrevInput;
    toUseMomentumAngUpdate = passPrevInput2;
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    aDQ->gOrientationQuat = static_cast<float>(aTimeDelta) * newQ.gSpin;
    //aDQ->gAngularMomentum = static_cast<float>(aTimeDelta) * newQ.gAngularMomentum;
    //aDQ->gAngularMomentum = static_cast<float>(aTimeDelta) * newQ.gAngularVelocity;
...
void Vehicle::RightHandSide5(struct HeliData* aHeli, Motion* aQ, Motion* aDeltaQ, double aTimeDelta, float aQScale, Motion* aDQ)
{
    //  Compute the intermediate values of the 
    //  dependent variables.
    Motion newQ;
...
//    testAngularPos += (0.5f * aTimeDelta) * (testInputQuat * m_testOrientQuat);
    ...
    */
    ...
    //toUseMomentumAngUpdate = toUseMomentumAngUpdate;
    //toUseVelocityAngUpdate = toUseVelocityAngWorking;

    DirectX::SimpleMath::Quaternion toUseQuat = deltaTestQuat;
    DirectX::SimpleMath::Vector3 momentumUpdate;
    momentumUpdate.x = toUseQuat.x;
    momentumUpdate.y = toUseQuat.y;
    momentumUpdate.z = toUseQuat.z;
    momentumUpdate *= toUseQuat.w;
    //momUpdate = momentumUpdate;
...
{
    float prevAngularMomentum = m_heli.q.angularMomentumUpdateTest.Length();
    float prevAngularVelocity = m_heli.q.angularVelocityUpdateTest.Length();
    ...
    DirectX::SimpleMath::Quaternion gTestOrientationUpdate = (dq1.gTestOrientation + 2.0 * dq2.gTestOrientation + 2.0 * dq3.gTestOrientation + dq4.gTestOrientation) * (1.0 / numEqns);
    gTestOrientationUpdate.Normalize();

    DirectX::SimpleMath::Quaternion toUseOrientationUpdate = (dq1.toUseOrientation + 2.0 * dq2.toUseOrientation + 2.0 * dq3.toUseOrientation + dq4.toUseOrientation) * (1.0 / numEqns);
    toUseOrientationUpdate.Normalize();

    DirectX::SimpleMath::Quaternion toUseSpinUpdate = (dq1.toUseSpin + 2.0 * dq2.toUseSpin + 2.0 * dq3.toUseSpin + dq4.toUseSpin) * (1.0 / numEqns);
    toUseSpinUpdate.Normalize();

    DirectX::SimpleMath::Vector3 toUseAngularMomentumUpdate = (dq1.toUseAngularMomentum + 2.0 * dq2.toUseAngularMomentum + 2.0 * dq3.toUseAngularMomentum + dq4.toUseAngularMomentum) / numEqns;
    DirectX::SimpleMath::Vector3 toUseAngularVelocityUpdate = (dq1.toUseAngularVelocity + 2.0 * dq2.toUseAngularVelocity + 2.0 * dq3.toUseAngularVelocity + dq4.toUseAngularVelocity) / numEqns;
    
    DirectX::SimpleMath::Vector3 angularVelocityUpdateTest = (dq1.angularVelocityUpdateTest + 2.0 * dq2.angularVelocityUpdateTest + 2.0 * dq3.angularVelocityUpdateTest + dq4.angularVelocityUpdateTest) / numEqns;
    DirectX::SimpleMath::Vector3 angularMomentumUpdateTest = (dq1.angularMomentumUpdateTest + 2.0 * dq2.angularMomentumUpdateTest + 2.0 * dq3.angularMomentumUpdateTest + dq4.angularMomentumUpdateTest) / numEqns;

    DirectX::SimpleMath::Vector3 angAxis = DirectX::SimpleMath::Vector3::UnitY;
    //angAxis = angularVelocityVecUpdate;
    angAxis = angularPosVecUpdate;
    angAxis.Normalize();
    float angRate = 0.0f;
    //angRate = angularVelocityVecUpdate.Length();
    angRate = angularPosVecUpdate.Length();

    DirectX::SimpleMath::Quaternion angQuat = DirectX::SimpleMath::Quaternion::Identity;
    angQuat = aHeli->q.angularPosQuat;
    if (angAxis.Length() != 0.0f)
    {
        angQuat *= DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(angAxis, angRate);
    }
    //angularVelocityQuatUpdate = angQuat;
    ...
    q.angularVelocityQuat += angularVelocityQuatUpdate;
    q.angularVelocityQuat.Normalize();
    q.angularPosQuat += angularPosQuatUpdate;
    ...
    q.gTestOrientation += gTestOrientationUpdate;
    //q.gTestOrientation *= 0.5f;
    q.gTestOrientation.Normalize();
  
    q.toUseOrientation += toUseOrientationUpdate;
    //q.toUseOrientation *= 0.5f;
    q.toUseOrientation.Normalize();
    q.toUseSpin += toUseSpinUpdate;
    //q.toUseSpin *= 0.5f;
    q.toUseSpin.Normalize();
    q.toUseAngularMomentum += toUseAngularMomentumUpdate;
    q.toUseAngularVelocity += toUseAngularVelocityUpdate;
    q.angularVelocityUpdateTest += angularVelocityUpdateTest;
    q.angularMomentumUpdateTest += angularMomentumUpdateTest;

    int stepCount = m_stepCount;
    float timer = m_testTimer2;
    ...
    aHeli->q.gTestOrientation = q.gTestOrientation;
    aHeli->q.toUseOrientation = q.toUseOrientation;
    aHeli->q.toUseSpin = q.toUseSpin;
    aHeli->q.toUseAngularMomentum = q.toUseAngularMomentum;
    aHeli->q.toUseAngularVelocity = q.toUseAngularVelocity;
    aHeli->q.angularVelocityUpdateTest = q.angularVelocityUpdateTest;
    aHeli->q.angularMomentumUpdateTest = q.angularMomentumUpdateTest;
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    float postAngularMomentum = m_heli.q.angularMomentumUpdateTest.Length();
    float postAngularVelocity = m_heli.q.angularVelocityUpdateTest.Length();
    ...
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularQuatUpdateTest.x =  ", aHeli->q.angularQuatUpdateTest.x, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularQuatUpdateTest.y =  ", aHeli->q.angularQuatUpdateTest.y, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularQuatUpdateTest.z =  ", aHeli->q.angularQuatUpdateTest.z, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularQuatUpdateTest.w =  ", aHeli->q.angularQuatUpdateTest.w, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularQuatUpdateTest.Length() =  ", aHeli->q.angularQuatUpdateTest.Length(), "");

    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularMomentumUpdateTest.x =  ", aHeli->q.angularMomentumUpdateTest.x, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularMomentumUpdateTest.y =  ", aHeli->q.angularMomentumUpdateTest.y, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularMomentumUpdateTest.z =  ", aHeli->q.angularMomentumUpdateTest.z, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularMomentumUpdateTest.Length() =  ", aHeli->q.angularMomentumUpdateTest.Length(), "");

    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularVelocityUpdateTest.x =  ", aHeli->q.angularVelocityUpdateTest.x, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularVelocityUpdateTest.y =  ", aHeli->q.angularVelocityUpdateTest.y, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularVelocityUpdateTest.z =  ", aHeli->q.angularVelocityUpdateTest.z, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularVelocityUpdateTest.Length() =  ", aHeli->q.angularVelocityUpdateTest.Length(), "");

    m_debugData->DebugPushUILineDecimalNumber("aHeli->iq.angularVelocityUpdateTest.x   =  ", aHeli->q.angularVelocityUpdateTest.x, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.angularMomentumUpdateTest.x =  ", aHeli->q.angularMomentumUpdateTest.x, "");
    DirectX::SimpleMath::Quaternion testAngularPos = m_heli.q.angularPosQuat;// *static_cast<float>(aTimeDelta);
    testAngularPos.Normalize();
    testAngularPos *= m_heli.q.alignmentQuat;
    testAngularPos.Normalize();

    float angRate2 = Utility::ToRadians(30.0f);
    angRate2 *= static_cast<float>(aTimeDelta);
    DirectX::SimpleMath::Vector3 angAxis2 = DirectX::SimpleMath::Vector3::UnitX;
    DirectX::SimpleMath::Quaternion angQuat2 = DirectX::SimpleMath::Quaternion::Identity;

    angQuat2 *= DirectX::SimpleMath::Quaternion::CreateFromAxisAngle(angAxis2, angRate2);

    testAngularPos = m_heli.q.angularPosQuat;
    //testAngularPos = (angQuat2 * m_heli.q.alignmentQuat);// *0.5f;
    ...
    testAngularPos = m_heli.q.gTestOrientation;
    ...
    /*
    testAngularPos = m_testQuat;
    DirectX::SimpleMath::Quaternion updateQuat = DirectX::SimpleMath::Quaternion(1.5f, 0.0f, 0.0f, 1.0f);
    updateQuat.Normalize();
    testAngularPos += updateQuat;
    testAngularPos += updateQuat;
    testAngularPos.Normalize();
    m_testQuat = testAngularPos;
    testAngularPos.Normalize();

    DirectX::SimpleMath::Vector3 vAngularVelocity = m_prevInput2;
    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, m_testOrientQuat);
    ...
    testInput = m_testTorqueLocal;
    //testInput = m_testTorqueWorld;
    //testInput = DirectX::SimpleMath::Vector3(1.0f, 0.0f, 0.0f);
    //DirectX::SimpleMath::Vector3 momentsInput = testInput;
    //momentsInput += m_prevInput2;
    ...
    DirectX::SimpleMath::Vector3 momentsInput = testInput;
    //m_prevInput = testInput;
    m_debugData->DebugPushUILineDecimalNumber("testInput.x =  ", testInput.x, "");
    m_debugData->DebugPushUILineDecimalNumber("testInput.y =  ", testInput.y, "");
    m_debugData->DebugPushUILineDecimalNumber("testInput.z =  ", testInput.z, "");

    //////////////////////////////////////////////////////////////////////////////////
    //m_testInertiaTensorLocal = DirectX::SimpleMath::Matrix::Identity;
    //m_testInverseInertiaTensorLocal = DirectX::SimpleMath::Matrix::Identity;
    //DirectX::SimpleMath::Vector3 vAngularVelocity = m_heli.q.gAngularVelocity;
    DirectX::SimpleMath::Vector3 toCrossVelocity = vAngularVelocity;
    toCrossVelocity = DirectX::SimpleMath::Vector3::Transform(toCrossVelocity, m_testInertiaTensorLocal);
    ...
    testInputQuat.z = testInput.z;
    testInputQuat.w = 0.0f;
    //testInputQuat *= m_testOrientQuat;
    //testAngularPos = 0.5f * testInputQuat * m_testOrientQuat;
    //testAngularPos = 0.5f * testInputQuat * (m_testQuat + m_testOrientQuat);
    testAngularPos = 0.5f * testInputQuat * (m_testQuat * m_testOrientQuat);
    testAngularPos = 0.5f * (testInputQuat) * (m_testOrientQuat);
    //testAngularPos = 0.5f * testInputQuat * (m_testQuat);
    //testAngularPos = testInputQuat;
    //testAngularPos = (m_testOrientQuat * testInputQuat) * (0.5f * aTimeDelta);
    //m_testQuat = testAngularPos * m_testOrientQuat;
    testAngularPos += m_testOrientQuat;
    testAngularPos.Normalize();
    
    m_debugData->DebugPushUILineDecimalNumber("testAngularPos.x =  ", testAngularPos.x, "");
    m_debugData->DebugPushUILineDecimalNumber("testAngularPos.y =  ", testAngularPos.y, "");
    m_debugData->DebugPushUILineDecimalNumber("testAngularPos.z =  ", testAngularPos.z, "");
    m_debugData->DebugPushUILineDecimalNumber("testAngularPos.w =  ", testAngularPos.w, "");

    m_debugData->DebugPushUILineDecimalNumber("m_testQuat.x =  ", m_testQuat.x, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testQuat.y =  ", m_testQuat.y, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testQuat.z =  ", m_testQuat.z, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testQuat.w =  ", m_testQuat.w, "");

    m_debugData->DebugPushUILineDecimalNumber("m_testOrientQuat.x =  ", m_testOrientQuat.x, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testOrientQuat.y =  ", m_testOrientQuat.y, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testOrientQuat.z =  ", m_testOrientQuat.z, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testOrientQuat.w =  ", m_testOrientQuat.w, "");
    */
    DirectX::SimpleMath::Quaternion prevQuat = m_heli.q.alignmentQuat;
    //testAngularPos.Normalize();
    //testAngularPos *= m_heli.q.alignmentQuat;
    //testAngularPos.Normalize();
    //m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_heli.q.angularPosQuat);
    //m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(testAngularPos);
    ///////////////////////////////////////////////////////////////////
    /*
    //m_testOrientQuat.Normalize();
    DirectX::SimpleMath::Quaternion testInputQuat;
    //testInput = DirectX::SimpleMath::Vector3::Transform(testInput, m_testOrientQuat);
    //m_prevInput = testInput;
    DirectX::SimpleMath::Vector3 testInput = aHeli->q.toUseAngularVelocity;
    //testInput = momentsInput;
    testInputQuat.x = testInput.x;
    testInputQuat.y = testInput.y;
    testInputQuat.z = testInput.z;
    ...
    //testAngularPos = 0.5f * (testInputQuat * m_testOrientQuat);
    //DirectX::SimpleMath::Quaternion  testAngularPos = m_testOrientQuat;
    DirectX::SimpleMath::Quaternion  testAngularPos2 = aHeli->q.toUseOrientation;
    //testAngularPos2 += (0.5f * aTimeDelta) * (testInputQuat * m_testOrientQuat);
    testAngularPos2 += (0.5f * aTimeDelta) * (testInputQuat * aHeli->q.toUseOrientation);
    testAngularPos2.Normalize();
    aHeli->q.toUseOrientation = testAngularPos2;
    */
    ///////////////////////////////////////////////////////////////////
    /*
    testInputQuat.x = testInput.x;
...
//    testAngularPos += (0.5f * aTimeDelta) * (testInputQuat * m_testOrientQuat);
    ...
    testAngularPos.Normalize();
        ...
        aHeli->q.angularVelocityUpdateTest;
    */
    DirectX::SimpleMath::Quaternion updateAlignInputQuat;
    updateAlignInputQuat.x = aHeli->q.angularVelocityUpdateTest.x;
    updateAlignInputQuat.y = aHeli->q.angularVelocityUpdateTest.y;
    updateAlignInputQuat.z = aHeli->q.angularVelocityUpdateTest.z;
    updateAlignInputQuat.w = 0.0f;
    //updateAlignInputQuat.Normalize();
    DirectX::SimpleMath::Quaternion updateAlignOutputQuat = aHeli->q.angularQuatUpdateTest;
    updateAlignOutputQuat.Normalize();
    aHeli->q.angularQuatUpdateTest.Normalize();
    updateAlignOutputQuat += (0.5f * aTimeDelta) * (updateAlignInputQuat * aHeli->q.angularQuatUpdateTest);
    //updateAlignOutputQuat += (aTimeDelta) * (updateAlignInputQuat * aHeli->q.angularQuatUpdateTest);
    updateAlignOutputQuat.Normalize();
    aHeli->q.angularQuatUpdateTest = updateAlignOutputQuat;
    ///////////////////////////////////////////////////////////////////
   // m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(aHeli->q.toUseOrientation);
    m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(aHeli->q.angularQuatUpdateTest);

    m_testOrientQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
    m_testOrientQuat.Normalize();
    m_heli.q.alignmentQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
    m_heli.q.alignmentQuat.Normalize();

    float angleDelta = DirectX::SimpleMath::Quaternion::Angle(prevQuat, m_heli.q.alignmentQuat);
    float angleDeltaDeg = angleDelta;
    angleDeltaDeg = Utility::ToDegrees(angleDeltaDeg);
    angleDelta /= aTimeDelta;
    ...
    float inertia = m_testInertiaFloat;
    float invInertia = m_testInverseInertiaFloat;
    m_rotPerSecDeg = angleDeltaDeg;
    m_rotPerSecRad = angleDelta;


    float mom1 = dq1.toUseAngularMomentum.x;
    float mom2 = dq2.toUseAngularMomentum.x;
    float mom3 = dq3.toUseAngularMomentum.x;
    float mom4 = dq4.toUseAngularMomentum.x;
    //m_heli.q.toUseAngularMomentum = DirectX::SimpleMath::Vector3(598.0, 0.0, 0.0);
    ...
    m_debugData->PushDebugLine(m_heli.q.position, m_heli.q.angularMomentumUpdateTest, 10.0f, 0.0f, DirectX::Colors::Red);
    m_debugData->PushDebugLine(m_heli.q.position, m_heli.q.angularVelocityUpdateTest, 20.0f, 0.0f, DirectX::Colors::Blue);
    m_heli.alignmentInverse = m_heli.alignment;
    m_heli.alignmentInverse.Invert();
}


void Vehicle::TempTestRungeKutta42(struct HeliData* aHeli, double aTimeDelta)
{
    DirectX::SimpleMath::Vector3 vAngularVelocity = m_prevInput2;
    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, m_testOrientQuat);
    ...
    m_debugData->PushDebugLine(m_heli.q.position, DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, testAngularPos), 10.0, 0.0f, DirectX::Colors::Red);
    m_debugData->PushDebugLine(m_heli.q.position, DirectX::SimpleMath::Vector3::Transform(momentsInput, testAngularPos), 10.0, 0.0f, DirectX::Colors::Blue);
...
void Vehicle::TempTestRungeKutta4(struct HeliData* aHeli, double aTimeDelta)
{
    //DirectX::SimpleMath::Vector3 vAngularVelocity = m_prevInput2;
    DirectX::SimpleMath::Vector3 vAngularVelocity = m_angVel;
    DirectX::SimpleMath::Vector3 vAngularVelocity2 = m_angVel;
    DirectX::SimpleMath::Vector3 vAngularVelocity3 = m_angVel;
    vAngularVelocity = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity, m_testOrientQuat);
    vAngularVelocity3 = DirectX::SimpleMath::Vector3::Transform(vAngularVelocity3, m_testOrientQuat);
    DirectX::SimpleMath::Vector3 testInput = DirectX::SimpleMath::Vector3::Zero;
    testInput.x = -m_heli.controlInput.cyclicInputRoll;
    testInput.y = -m_heli.controlInput.yawPedalInput;
    testInput.z = -m_heli.controlInput.cyclicInputPitch;
    ...
    */
    ...
    //testAngularPos += (0.5f) * (testInputQuat * m_testOrientQuat);
    //testAngularPos += (m_testOrientQuat * testInputQuat) * (0.5f * aTimeDelta);
    //testAngularPos += (m_testOrientQuat * testInputQuat) * (0.5f);
    //testAngularPos += (testInputQuat * m_testOrientQuat) * (0.5f);
    //testAngularPos += m_testOrientQuat;
    testAngularPos.Normalize();
    ...
    DirectX::SimpleMath::Vector3 testMom = m_angMom;
    DirectX::SimpleMath::Vector3 testVel = m_angVel;
    float inertia = m_testInertiaFloat;
    float invInertia = m_testInverseInertiaFloat;
    m_rotPerSecDeg = angleDeltaDeg;
    m_rotPerSecRad = angleDelta;
    ...
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.m_angMom.x =  ", m_angMom.x, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.m_angMom.y =  ", m_angMom.y, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.m_angMom.z =  ", m_angMom.z, "");
    m_debugData->DebugPushUILineDecimalNumber("aHeli->q.m_angMom.Length() =  ", m_angMom.Length(), "");
}

void Vehicle::RungeKutta42(struct HeliData* aHeli, double aTimeDelta)
{
    //  Define a convenience variables
    const float numEqns = static_cast<float>(aHeli->numEqns);
    //  Retrieve the current values of the dependent and independent variables.
    ...
    if (m_debugToggle7 == true)
    {
        //angAxis = DirectX::SimpleMath::Vector3::UnitZ;
    }
    if (m_debugToggle8 == false)
    {
        //angRate *= -1.0f;
    ...
    if (m_testTimer2 > 10.0f)
    {
        ...
        float angDegPerSec = m_testValDebug2;
        float angDegPersec1 = m_testValDebug3;

        int testBreak = 0;
        testBreak++;
    }
    //angAxis = DirectX::SimpleMath::Vector3::Transform(angAxis, m_heli.alignment);
    DirectX::SimpleMath::Quaternion angQuat = DirectX::SimpleMath::Quaternion::Identity;
    angQuat = aHeli->q.angularPosQuat;
    //angQuat = aHeli->q.angularVelocityQuat;
    //angQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
    ...
    if (m_debugToggle5 == false)
    {
        ...
        //angularPosQuatUpdate = angQuat;

        //angularVelocityQuatUpdate = DirectX::SimpleMath::Quaternion::Identity;
        //angularPosQuatUpdate = angQuat;
        //angularPosQuatUpdate = DirectX::SimpleMath::Quaternion::Identity;


        DirectX::SimpleMath::Quaternion testAngPosQuat = DirectX::SimpleMath::Quaternion::Identity;
        testAngPosQuat = aHeli->q.angularPosQuat;
        //testAngPosQuat *= angularVelocityQuatUpdate;
        testAngPosQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
        testAngPosQuat *= angQuat;
        //angularPosQuatUpdate = angQuat;
        //angularPosQuatUpdate = testAngPosQuat;


        DirectX::SimpleMath::Quaternion testAngAccQuat = DirectX::SimpleMath::Quaternion::Identity;
        //testAngAccQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
        //testAngAccQuat *= aHeli->q.angularPosQuat;
        testAngAccQuat = aHeli->q.angularVelocityQuat;
        //testAngAccQuat *= angularVelocityQuatUpdate;
        //testAngAccQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
        //testAngAccQuat += angQuat;
        testAngAccQuat = angQuat;
        //angularVelocityQuatUpdate = angQuat;
        angularVelocityQuatUpdate = testAngAccQuat;




        testAngAccQuat = DirectX::SimpleMath::Quaternion::Identity;
        testAngAccQuat = aHeli->q.angularPosQuat;
        //testAngAccQuat += angularVelocityQuatUpdate;
        testAngAccQuat = angQuat;
        angularVelocityQuatUpdate = testAngAccQuat;
        ...
        testAngPosQuat = DirectX::SimpleMath::Quaternion::Identity;
        //testAngPosQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
        testAngPosQuat *= aHeli->q.angularPosQuat;
        //testAngPosQuat *= aHeli->q.angularVelocityQuat;
        //testAngPosQuat *= angularVelocityQuatUpdate;
        //angularPosQuatUpdate = testAngPosQuat;
    }
    else
    ...
    /*
    angularPosQuatUpdate = angQuat;
    //angularPosVecUpdate = DirectX::SimpleMath::Vector3::Transform(angularPosVecUpdate, angularQuatUpdate);
    ...
    */
    ...
    q.angularVelocityVec += angularVelocityVecUpdate;
    q.angularVelocityQuat += angularVelocityQuatUpdate;
    //q.angularVelocityQuat += testAngAccQuat;
    q.angularVelocityQuat.Normalize();
    q.angularPosQuat += angularPosQuatUpdate;
    //q.angularPosQuat += testAngAccQuat;
    q.angularPosQuat.Normalize();

    //q.angPosVec = DirectX::SimpleMath::Vector3::Transform(q.angPosVec, angularPosQuatUpdate);
    //q.angularVelocityVec = DirectX::SimpleMath::Vector3::Transform(q.angularVelocityVec, angularPosQuatUpdate);
...
void Vehicle::RungeKutta43(struct HeliData* aHeli, double aTimeDelta)
{
    ...
    testInput.y = -m_heli.controlInput.yawPedalInput;
    testInput.z = -m_heli.controlInput.cyclicInputPitch;
    //testInput = DirectX::SimpleMath::Vector3(0.4523f, 0.0f, 0.0f);
    DirectX::SimpleMath::Vector3 momentsInput = testInput;
    momentsInput += m_prevInput2;
...
void Vehicle::RungeKutta4Working(struct HeliData* aHeli, double aTimeDelta)
{
    float prevAngularMomentum = m_heli.q.gAngularMomentum.Length();
    float prevAngularVelocity = m_heli.q.gAngularVelocity.Length();
...
void Vehicle::UpdateAlignmentTorque()
{
    ...
    if (testAngVec != DirectX::SimpleMath::Vector3::Zero)
    {
        ...
        //m_heli.alignment = DirectX::SimpleMath::Matrix::Transform(DirectX::SimpleMath::Matrix::Identity, testQuat);
    }
    DirectX::SimpleMath::Quaternion testAngularPos = m_heli.q.angularPosQuat;
    testAngularPos *= m_heli.q.alignmentQuat;
    //m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(m_heli.q.angularPosQuat);
    m_heli.alignment = DirectX::SimpleMath::Matrix::CreateFromQuaternion(testAngularPos);
    m_heli.q.alignmentQuat = DirectX::SimpleMath::Quaternion::CreateFromRotationMatrix(m_heli.alignment);
...
void Vehicle::UpdateAlignmentTorque2()
{
    m_heli.cameraOrientationPrevious = m_heli.cameraOrientation;
...
Utility::Torque Vehicle::UpdateBodyTorqueRunge2(DirectX::SimpleMath::Vector3& aAccelVec, Utility::Torque aPendTorque, const float aTimeStep)
{
    ...
    */
    ...
    /*
    m_debugData->PushDebugLine(m_heli.q.position, torqueVec, 10.0f, 0.0f, DirectX::Colors::White);
    m_debugData->PushDebugLine(m_heli.q.position, tailVec, 10.0f, 0.0f, DirectX::Colors::Red);
    m_debugData->PushDebugLine(m_heli.q.position, gravVec, 10.0f, 0.0f, DirectX::Colors::Green);
    m_debugData->PushDebugLine(m_heli.q.position, driveVec, 10.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->PushDebugLine(m_heli.q.position, terrainVec, 10.0f, 0.0f, DirectX::Colors::Yellow);
    ...
    m_debugData->DebugPushUILineDecimalNumber("torqueVec.Length()  ", torqueVec.Length(), "");
    */
...
void Vehicle::UpdateTestDrivetrainTorque4(const float aTimer)
{
    ...
    /*
    DirectX::SimpleMath::Vector3 testNorm = DirectX::SimpleMath::Vector3::UnitY + m_heli.up;
    testNorm.Normalize();
    lateralForce = (m_heli.controlInput.cyclicInputRoll * torqueMax) * testNorm;
    longitudinalForce = (m_heli.controlInput.cyclicInputPitch * torqueMax) * testNorm;
    m_debugData->PushDebugLine(m_heli.q.position, testNorm, 10.0f, 0.0f, DirectX::Colors::Yellow);
    */
    ...
    m_debugData->PushDebugLineScaled(m_heli.centerOfMass, lateralForce, 2.0f, 1.0f, 0.0f, DirectX::Colors::Blue);
    ...
    */
    ...
    if (m_debugToggle == true && 1 == 1)
    {
        if (m_heli.controlInput.cyclicInputRoll > 0.0f) // left
        {
            if (m_heli.physicsPointLeft.y < m_heli.physicsPointRight.y)
            {
                float preMag = lateralTorque.magnitude;
                lateralTorque.magnitude *= (1.0f - lateralDot);
                lateralTorqueLocal.magnitude *= (1.0f - lateralDot);
                float postMag = lateralTorque.magnitude;
                modDelta = postMag - preMag;
                m_debugData->DebugPushUILineDecimalNumber("DebugLeft ", modDelta, "");
            }
        }
        else if (m_heli.controlInput.cyclicInputRoll < 0.0f) // right
        {
            if (m_heli.physicsPointLeft.y > m_heli.physicsPointRight.y)
            {
                ...
                float postMag = lateralTorque.magnitude;
                modDelta = postMag - preMag;
                m_debugData->DebugPushUILineDecimalNumber("DebugRight ", modDelta, "");
            }
        }
    ...
    m_debugData->PushDebugLinePositionIndicator(m_heli.physicsPointLeft, 5.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->PushDebugLinePositionIndicator(m_heli.physicsPointRight, 5.0f, 0.0f, DirectX::Colors::Red);

    //m_debugData->DebugPushUILineDecimalNumber("lateralTorque.magnitude      = ", lateralTorque.magnitude, "");
    //m_debugData->DebugPushUILineDecimalNumber("lateralTorqueLocal.magnitude = ", lateralTorqueLocal.magnitude, "");
...
void Vehicle::UpdateTestDrivetrainTorqueLastUsed(const float aTimer)
{
    DirectX::SimpleMath::Quaternion cyclicQuat = DirectX::SimpleMath::Quaternion::CreateFromYawPitchRoll(0.0f, m_heli.controlInput.cyclicInputRoll, m_heli.controlInput.cyclicInputPitch);
    DirectX::SimpleMath::Vector3 updatedCyclic = DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3::UnitY, cyclicQuat);
    float cyclicAngle = Utility::GetAngleBetweenVectors(DirectX::SimpleMath::Vector3::UnitY, updatedCyclic);
...
void Vehicle::UpdateTestDrivetrainTorque5(const float aTimer)
{
    DirectX::SimpleMath::Quaternion cyclicQuat = DirectX::SimpleMath::Quaternion::CreateFromYawPitchRoll(0.0f, m_heli.controlInput.cyclicInputRoll, m_heli.controlInput.cyclicInputPitch);
    DirectX::SimpleMath::Vector3 updatedCyclic = DirectX::SimpleMath::Vector3::Transform(DirectX::SimpleMath::Vector3::UnitY, cyclicQuat);
    float cyclicAngle = Utility::GetAngleBetweenVectors(DirectX::SimpleMath::Vector3::UnitY, updatedCyclic);
...
void Vehicle::UpdateVehicle(const double aTimeDelta)
{
    ...
    else if (m_testTimer2 < 0.0f)
    {
        m_testBoolFlipRot = false;
    }
    if (m_testBoolFlipRot == false)
    {
        m_testTimer2 += aTimeDelta;
    }
    else
    ...
    */
    DirectX::SimpleMath::Vector3 preUp = m_heli.up;
    ...
    if (m_isRungeOn == true)
    {
        ...
        //TempTestRungeKutta4(&m_heli, aTimeDelta);
    }
    ...
    */
    ...
    /*
    DirectX::SimpleMath::Vector3 posPoint = m_heli.q.position;
    posPoint.x += m_inertiaModelX;
    m_debugData->PushDebugLinePositionIndicator(posPoint, 5.0f, 0.0f, DirectX::Colors::Red);
    posPoint = m_heli.q.position;
    posPoint.y += m_inertiaModelY;
    m_debugData->PushDebugLinePositionIndicator(posPoint, 5.0f, 0.0f, DirectX::Colors::Red);
    posPoint = m_heli.q.position;
    posPoint.z += m_inertiaModelZ;
    m_debugData->PushDebugLinePositionIndicator(posPoint, 5.0f, 0.0f, DirectX::Colors::Red);

    posPoint = m_heli.q.position;
    posPoint.x -= m_inertiaModelX;
    m_debugData->PushDebugLinePositionIndicator(posPoint, 5.0f, 0.0f, DirectX::Colors::Blue);
    posPoint = m_heli.q.position;
    posPoint.y -= m_inertiaModelY;
    m_debugData->PushDebugLinePositionIndicator(posPoint, 5.0f, 0.0f, DirectX::Colors::Blue);
    posPoint = m_heli.q.position;
    ...
    */
    ...
    DirectX::SimpleMath::Vector3 postUp = m_heli.up;
    float angle = Utility::GetAngleBetweenVectors(preUp, postUp);
    float angleDegrees = Utility::ToDegrees(angle);
    float degPerSecond = angleDegrees / static_cast<float>(aTimeDelta);
    float radPerSecond2 = angle / static_cast<float>(aTimeDelta);
    m_debugData->DebugPushUILineDecimalNumber("angle delta per sec =  ", degPerSecond, "");
    m_debugData->DebugPushUILineDecimalNumber("rad delta per sec2 =  ", radPerSecond2, "");
    m_testValDebug1 = radPerSecond2;
    m_testValDebug2 = degPerSecond;
    float rotAngle = Utility::GetAngleBetweenVectors(DirectX::SimpleMath::Vector3::UnitZ, m_heli.right);
    float rotAngleDeg = Utility::ToDegrees(rotAngle);
    m_debugData->DebugPushUILineDecimalNumber("rotAngleDeg =  ", rotAngleDeg, "");

    float angDelta = rotAngle - m_prevAngle;

    if (rotAngleDeg > m_angleSwingMaxTest)
    {
        m_angleSwingMaxTest = rotAngleDeg;
    }
    else if (rotAngleDeg < 5.0f)
    {
        //m_angleSwingMaxTest4 = m_angleSwingMaxTest3;
        //m_angleSwingMaxTest3 = m_angleSwingMaxTest;
        //m_angleSwingMaxTest2 = 0.0f;
    }

    if (rotAngleDeg > m_angleSwingMaxTest2)
    {
        m_angleSwingMaxTest2 = rotAngleDeg;
        //m_angleSwingMaxTest = 0.0f;
    }
    else if (rotAngleDeg < 5.0)
    {
        //m_angleSwingMaxTest4 = m_angleSwingMaxTest2;
        //m_angleSwingMaxTest = 0.0f;
    }

    if (m_isGoingUp == true && angDelta < 0.0f)
    {
        if (m_isGoingUp == true)
        {
            if (m_isSwingToggleTrue == true)
            {
                m_isSwingToggleTrue = false;
                m_delta = m_angleSwingMaxTest3 - m_angleSwingMaxTest;
                m_deltaDelta = m_delta - m_delta2;
                m_angleSwingMaxTest3 = m_angleSwingMaxTest;
                m_angleSwingMaxTest = 0.0f;
                m_testValDebug4 = m_testTimer;
                m_testTimer = 0.0f;
            }
            else
            {
                m_isSwingToggleTrue = true;
                m_delta2 = m_angleSwingMaxTest4 - m_angleSwingMaxTest2;
                m_deltaDelta = m_delta2 - m_delta;
                m_angleSwingMaxTest4 = m_angleSwingMaxTest2;
                m_angleSwingMaxTest2 = 0.0f;
                m_testValDebug5 = m_testTimer3;
                m_testTimer3 = 0.0f;
            }
        }
        m_isGoingUp = false;
    }
    else if (m_isGoingUp == false && angDelta > 0.0f)
    {
        m_isGoingUp = true;
    }
    ...
    */
    ...
    {
        m_debugData->DebugPushUILineDecimalNumber("swingToggle false =  ", angDelta, "");
    }

    m_debugData->DebugPushUILineDecimalNumber("m_testValDebug4 =  ", m_testValDebug4, "");
    m_debugData->DebugPushUILineDecimalNumber("m_testValDebug5 =  ", m_testValDebug5, "");

    m_debugData->DebugPushUILineDecimalNumber("m_angleSwingMaxTest =  ", m_angleSwingMaxTest, "");
    m_debugData->DebugPushUILineDecimalNumber("m_angleSwingMaxTest2 =  ", m_angleSwingMaxTest2, "");
    m_debugData->DebugPushUILineDecimalNumber("m_angleSwingMaxTest3 =  ", m_angleSwingMaxTest3, "");
    m_debugData->DebugPushUILineDecimalNumber("m_angleSwingMaxTest4 =  ", m_angleSwingMaxTest4, "");

    m_debugData->DebugPushUILineDecimalNumber("m_delta      =  ", m_delta, "");
    m_debugData->DebugPushUILineDecimalNumber("m_delta2     =  ", m_delta2, "");
    m_debugData->DebugPushUILineDecimalNumber("m_deltaDelta =  ", m_deltaDelta, "");
    m_debugData->DebugPushUILineDecimalNumber("angDelta =  ", angDelta, "");
    ...
    DirectX::SimpleMath::Vector3 cordOrigin = DirectX::SimpleMath::Vector3::Zero;
    cordOrigin.x -= 3.0f;
    cordOrigin.y += 1.0f;
    cordOrigin.z -= 3.0f;
    m_debugData->PushDebugLine(cordOrigin, DirectX::SimpleMath::Vector3::UnitX, 50.0f, 0.0f, DirectX::Colors::White);
...
void Vehicle::UpdateVehicleForces(const float aTimeStep)
{
    ...
    */
    ...
    DirectX::SimpleMath::Vector3 testVec = m_testDrivetrainTorque.axis * m_testDrivetrainTorque.magnitude;
    if (m_heli.selfRightingForce != testVec)
    {
        int testBreak = 0;
        testBreak++;
    }
    ...
    DirectX::SimpleMath::Vector3 localAngularVec = UpdateBodyTorqueLocal(accelVecUpdate, pendTorque, static_cast<float>(aTimeStep));

    m_testTorqueLocal = localAngularVec;
    m_testTorqueWorld = accelVecUpdate;
    ...
    if (m_debugToggle == false)
    {
        angAccelVecTensorUpdate = DirectX::SimpleMath::Vector3::Transform(angAccelVecTensorUpdate, m_testTensor);
    }
    if (m_debugToggle2 == true)
    {
        angAccelVecTensorUpdate = DirectX::SimpleMath::Vector3::Transform(angAccelVecTensorUpdate, m_heli.alignment);
    ...
    if (m_debugToggle == false)
    {
        localAngularVec = DirectX::SimpleMath::Vector3::Transform(localAngularVec, m_testTensor);
    }
    if (m_debugToggle2 == true)
    {
        localAngularVec = DirectX::SimpleMath::Vector3::Transform(localAngularVec, m_heli.alignment);
    }
    ...
    m_debugData->DebugPushUILineDecimalNumber("m_heli.vehicleAngularForcesSum.Length() = ", m_heli.vehicleAngularForcesSum.Length(), "");
    m_stepCount++;
}
...
DirectX::SimpleMath::Vector3 Vehicle::UpdateBodyTorqueLocal(DirectX::SimpleMath::Vector3& aAccelVec, Utility::Torque aPendTorque, const float aTimeStep)
{
    ...
    //DirectX::SimpleMath::Vector3 gravityForce = m_heli.gravity * m_gravTorqueModTest;
    DirectX::SimpleMath::Vector3 gravityForce = m_heli.gravity;
    //DirectX::SimpleMath::Vector3 gravityForce = -m_heli.up * 9.8f;
    gravityForce = DirectX::SimpleMath::Vector3::Transform(gravityForce, m_heli.alignment);
    gravityForce *= m_gravTorqueModTest;
    DirectX::SimpleMath::Vector3 gravityForceWorld = gravityForce;
    gravityForceWorld = DirectX::SimpleMath::Vector3::Transform(gravityForceWorld, m_heli.alignmentInverse);
    //m_debugData->PushDebugLine(m_heli.q.position, gravityForce, 10.0f, 0.0f, DirectX::Colors::Red);

    DirectX::SimpleMath::Vector3 gravityTorqueArmWorld = gravityTorqueArm;
    gravityTorqueArmWorld = DirectX::SimpleMath::Vector3::Transform(gravityTorqueArmWorld, m_heli.alignment);
    /*
    m_debugData->PushDebugLineScaled(m_cordOrgTestPos, gravityTorqueArmWorld, 0.0f, 1.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmWorld, gravityForceWorld, 10.0f, 0.0f, DirectX::Colors::Red);
    m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmWorld, m_heli.gravity, 7.0f, 0.0f, DirectX::Colors::Yellow);
    */
    //Utility::Torque gravTorque = Utility::GetTorqueForce(gravityTorqueArm, gravityForce);
    Utility::Torque gravTorque = Utility::GetTorqueForce(gravityTorqueArmWorld, m_heli.gravity * m_gravTorqueModTest);
    gravityTorqueArmWorld *= 5.0f;
    m_cordOrgTestPos = m_heli.q.position;
    //m_debugData->PushDebugLineScaled(m_cordOrgTestPos, gravityTorqueArmWorld, 0.0f, 1.0f, 0.0f, DirectX::Colors::Blue);
    //m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmWorld, m_heli.gravity, 7.0f, 0.0f, DirectX::Colors::Yellow);


    gravityTorqueArmWorld = DirectX::SimpleMath::Vector3::Transform(gravityTorqueArmWorld, m_heli.alignment);
    DirectX::SimpleMath::Vector3 gravForceWorld = DirectX::SimpleMath::Vector3::Transform(-DirectX::SimpleMath::Vector3::UnitY, m_heli.alignment);
    m_debugData->PushDebugLineScaled(m_cordOrgTestPos, gravityTorqueArmWorld, 0.0f, 1.0f, 0.0f, DirectX::Colors::Teal);
    m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmWorld, gravForceWorld, 7.0f, 0.0f, DirectX::Colors::White);


    DirectX::SimpleMath::Vector3 gravForceLocal = -m_heli.gravity * m_gravTorqueModTest;
    gravForceLocal = DirectX::SimpleMath::Vector3::Transform(gravForceLocal, m_heli.alignment);
    DirectX::SimpleMath::Vector3 gravityTorqueArmLocal = m_heli.localCenterOfMass - m_heli.localGravityTorqueArmPos;

    DirectX::SimpleMath::Vector3 gravityTorqueArmTest = m_heli.localCenterOfMass - m_heli.localGravityTorqueArmPos;
    gravityTorqueArmTest = DirectX::SimpleMath::Vector3::Transform(gravityTorqueArmTest, m_heli.alignment);
    DirectX::SimpleMath::Vector3 gravForceTest = -m_heli.gravity * m_gravTorqueModTest;
    gravForceTest = DirectX::SimpleMath::Vector3::Transform(gravForceTest, m_heli.alignment);

    DirectX::SimpleMath::Quaternion testQuat = m_heli.q.angularPosQuat;
    //m_heli.q.inverseAlignmentQuat.Inverse(m_heli.q.inverseAlignmentQuat);
    //testQuat.Inverse(testQuat);
    gravityTorqueArmLocal = DirectX::SimpleMath::Vector3::Transform(gravityTorqueArmLocal, testQuat);
    gravForceLocal = DirectX::SimpleMath::Vector3::Transform(gravForceLocal, testQuat);
    Utility::Torque gravTorque2 = Utility::GetTorqueForce(gravityTorqueArmLocal, gravForceLocal);
    //Utility::Torque gravTorque2 = Utility::GetTorqueForce(gravityTorqueArmTest, gravForceTest);

    gravityTorqueArmLocal *= 5.0f;
    m_debugData->PushDebugLineScaled(m_cordOrgTestPos, gravityTorqueArmLocal, 0.0f, 1.0f, 0.0f, DirectX::Colors::Green);
    m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmLocal, gravForceLocal, 7.0f, 0.0f, DirectX::Colors::Red);

    gravityTorqueArmLocal = DirectX::SimpleMath::Vector3::Transform(gravityTorqueArmLocal, m_heli.alignment);
    gravForceLocal = DirectX::SimpleMath::Vector3::Transform(gravForceLocal, m_heli.alignment);
    //m_debugData->PushDebugLineScaled(m_cordOrgTestPos, gravityTorqueArmLocal, 0.0f, 1.0f, 0.0f, DirectX::Colors::Lime);
    //m_debugData->PushDebugLine(m_cordOrgTestPos + gravityTorqueArmLocal, gravForceLocal, 7.0f, 0.0f, DirectX::Colors::Orange);


    DirectX::SimpleMath::Vector3 gravTorqueWorld = gravTorque.axis;
    gravTorqueWorld = DirectX::SimpleMath::Vector3::Transform(gravTorqueWorld, m_heli.alignment);
    DirectX::SimpleMath::Vector3 gravTorqueWorld2 = gravTorque2.axis;
    gravTorqueWorld2 = DirectX::SimpleMath::Vector3::Transform(gravTorqueWorld2, m_heli.alignment);
    m_debugData->PushDebugLine(m_cordOrgTestPos, gravTorqueWorld, 13.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->PushDebugLine(m_cordOrgTestPos, gravTorqueWorld2, 7.0f, 0.0f, DirectX::Colors::Red);

    m_debugData->DebugPushUILineDecimalNumber("gravTorque.mag  =", gravTorque.magnitude, "");
    m_debugData->DebugPushUILineDecimalNumber("gravTorque.mag2 =", gravTorque2.magnitude, "");
    ...
    DirectX::SimpleMath::Vector3 armForward = DirectX::SimpleMath::Vector3::UnitX;
    DirectX::SimpleMath::Vector3 armRight = DirectX::SimpleMath::Vector3::UnitZ;
    DirectX::SimpleMath::Vector3 armRear = -DirectX::SimpleMath::Vector3::UnitX;
    DirectX::SimpleMath::Vector3 armLeft = -DirectX::SimpleMath::Vector3::UnitZ;
    armForward = DirectX::SimpleMath::Vector3::Transform(armForward, m_heli.alignment);
    armRight = DirectX::SimpleMath::Vector3::Transform(armRight, m_heli.alignment);
    armRear = DirectX::SimpleMath::Vector3::Transform(armRear, m_heli.alignment);
    armLeft = DirectX::SimpleMath::Vector3::Transform(armLeft, m_heli.alignment);

    float leftRightDelta = armLeft.y - armRight.y;
    float rightLeftDelta = armRight.y - armLeft.y;
    float forwardRearDelta = armForward.y - armRear.y;
    float rearForwardDelta = armRear.y - armForward.y;
    leftRightDelta *= 0.25f;
    forwardRearDelta *= 0.25f;
    rightLeftDelta *= 0.25f;
    rearForwardDelta *= 0.25f;

    leftRightDelta += 0.5f;
    forwardRearDelta += 0.5f;
    rightLeftDelta += 0.5f;
    rearForwardDelta += 0.5f;

    //armForward = DirectX::SimpleMath::Vector3::UnitX;
    //armRight = DirectX::SimpleMath::Vector3::UnitZ;
    //armRear = -DirectX::SimpleMath::Vector3::UnitX;
    //armLeft = -DirectX::SimpleMath::Vector3::UnitZ;
    m_debugData->DebugClearUI();
    m_debugData->DebugPushUILineDecimalNumber("leftRightDelta = ", leftRightDelta, "");
    m_debugData->DebugPushUILineDecimalNumber("rightLeftDelta = ", rightLeftDelta, "");
    m_debugData->DebugPushUILineDecimalNumber("forwardRearDelta = ", forwardRearDelta, "");
    m_debugData->DebugPushUILineDecimalNumber("rearForwardDelta = ", rearForwardDelta, "");
    //DirectX::SimpleMath::Vector3 testGravForce = m_heli.up * (9.8f * (m_gravTorqueModTest * 0.25f));
    DirectX::SimpleMath::Vector3 testGravForce = DirectX::SimpleMath::Vector3::UnitY * (9.8f * (m_gravTorqueModTest * 0.1f));
    DirectX::SimpleMath::Vector3 forwardForce = testGravForce;
    DirectX::SimpleMath::Vector3 rearForce = testGravForce;
    DirectX::SimpleMath::Vector3 leftForce = testGravForce;
    DirectX::SimpleMath::Vector3 rightForce = testGravForce;

    forwardForce *= (1.0f - forwardRearDelta);
    rearForce *= (1.0f - rearForwardDelta);
    leftForce *= (1.0f - leftRightDelta);
    rightForce *= (1.0f - rightLeftDelta);

    Utility::Torque forwardTorque = Utility::GetTorqueForce(armForward, forwardForce);
    Utility::Torque rightTorque = Utility::GetTorqueForce(armRight, rightForce);
    Utility::Torque rearTorque = Utility::GetTorqueForce(armRear, rearForce);
    Utility::Torque leftTorque = Utility::GetTorqueForce(armLeft, leftForce);
    /*
    forwardTorque.magnitude *= (1.0f - longitudinalDot);
    rightTorque.magnitude *= (1.0f - lateralDot);
    rearTorque.magnitude *= (1.0f - longitudinalDot);
    leftTorque.magnitude *= (1.0f - lateralDot);
    */
    /*
    forwardTorque.magnitude *= (1.0f + forwardRearDelta);
    rightTorque.magnitude *= (1.0f - leftRightDelta);
    rearTorque.magnitude *= (1.0f - forwardRearDelta);
    leftTorque.magnitude *= (1.0f + leftRightDelta);
    */
    /*
    forwardTorque.magnitude *= (leftRightDelta);
    rightTorque.magnitude *= (-forwardRearDelta);
    rearTorque.magnitude *= (leftRightDelta);
    leftTorque.magnitude *= (forwardRearDelta);
    */
    Utility::Torque testGravTorque;
    testGravTorque.axis = (forwardTorque.axis * forwardTorque.magnitude) + (rightTorque.axis * rightTorque.magnitude) + (rearTorque.axis * rearTorque.magnitude) + (leftTorque.axis * leftTorque.magnitude);
    testGravTorque.magnitude = forwardTorque.magnitude + rightTorque.magnitude + rearTorque.magnitude + leftTorque.magnitude;

    m_debugData->PushDebugLine(m_cordOrgTestPos, testGravTorque.axis, 17.0f, 0.0f, DirectX::Colors::White);
    m_debugData->DebugPushUILineDecimalNumber("testGravTorque.mag  =", testGravTorque.magnitude, "");


    //gravTorque = testGravTorque;
    DirectX::SimpleMath::Quaternion accelQuat = m_heli.q.angularVelocityQuat;
    DirectX::SimpleMath::Quaternion velQuat = m_heli.q.angularPosQuat;
    DirectX::SimpleMath::Vector3 accelVec = m_heli.q.angularVelocityVec;
    accelVec = DirectX::SimpleMath::Vector3::Transform(accelVec, m_heli.alignment);
    DirectX::SimpleMath::Vector3 posVec = m_heli.q.angPosVec;
    posVec = DirectX::SimpleMath::Vector3::Transform(posVec, m_heli.alignment);

    m_debugData->DebugClearUI();
    m_debugData->PushDebugLine(m_cordOrgTestPos, accelVec, 7.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->PushDebugLine(m_cordOrgTestPos, posVec, 10.0f, 0.0f, DirectX::Colors::Red);


    DirectX::SimpleMath::Vector3 testVelVec = DirectX::SimpleMath::Vector3::UnitY;
    testVelVec = DirectX::SimpleMath::Vector3::Transform(testVelVec, m_heli.q.angularVelocityQuat);
    DirectX::SimpleMath::Vector3 testPosVec = DirectX::SimpleMath::Vector3::UnitY;
    testPosVec = DirectX::SimpleMath::Vector3::Transform(testPosVec, m_heli.q.angularPosQuat);
    m_debugData->PushDebugLine(m_cordOrgTestPos, testVelVec, 7.0f, 0.0f, DirectX::Colors::Yellow);
    m_debugData->PushDebugLine(m_cordOrgTestPos, testPosVec, 10.0f, 0.0f, DirectX::Colors::Orange);
    m_debugData->DebugClearUI();
    ...
    m_cordOrgTestPos = DirectX::SimpleMath::Vector3(6.0f, 10.0f, 9.0f);
    ...
    //DirectX::SimpleMath::Vector3 terrainVec = m_testTerrainNormTorque.axis; m_testDrivetrainTorqueLocal
...
Utility::Torque Vehicle::UpdateBodyTorqueRunge(DirectX::SimpleMath::Vector3& aAccelVec, Utility::Torque aPendTorque, const float aTimeStep)
{
    ...
    /*
    m_debugData->PushDebugLine(m_heli.q.position, torqueVec, 20.0f, 0.0f, DirectX::Colors::Teal);
    //m_debugData->PushDebugLine(m_heli.q.position, tailVec, 10.0f, 0.0f, DirectX::Colors::Red);
    m_debugData->PushDebugLine(m_heli.q.position, gravVec, 10.0f, 0.0f, DirectX::Colors::Green);
    m_debugData->PushDebugLine(m_heli.q.position, driveVec, 10.0f, 1.0f, DirectX::Colors::Orange);
    m_debugData->PushDebugLine(m_heli.q.position, terrainVec, 10.0f, 0.0f, DirectX::Colors::Yellow);
    m_debugData->PushDebugLine(m_heli.q.position, pendVec, 10.0f, 0.0f, DirectX::Colors::Blue);
    m_debugData->DebugPushUILineDecimalNumber("tailVec.Length()     ", tailVec.Length(), "");
    m_debugData->DebugPushUILineDecimalNumber("gravVec.Length()    ", gravVec.Length(), "");
    m_debugData->DebugPushUILineDecimalNumber("driveVec.Length()   ", driveVec.Length(), "");
    m_debugData->DebugPushUILineDecimalNumber("terrainVec.Length() ", terrainVec.Length(), "");
    m_debugData->DebugPushUILineDecimalNumber("pendVec.Length()    ", pendVec.Length(), "");
    m_debugData->DebugPushUILineDecimalNumber("torqueVec.Length()  ", torqueVec.Length(), "");
    */
...
void Vehicle::DebugInputVelocityZero()
{
    ...
    m_heli.q.angPosVec = DirectX::SimpleMath::Vector3::Zero;
    m_heli.q.angularVelocityVec = DirectX::SimpleMath::Vector3::Zero;
    m_heli.q.angularPosQuat = DirectX::SimpleMath::Quaternion::Identity;
    m_heli.q.angularVelocityQuat = DirectX::SimpleMath::Quaternion::Identity;
    m_heli.q.angularPosQuat = DirectX::SimpleMath::Quaternion::Identity;
}
